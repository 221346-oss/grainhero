...
const getUpstreamBase = () => {
  // Prefer server-side env. Keep NEXT_PUBLIC_BACKEND_URL for convenience.
  return (
...
async function proxy(req: NextRequest, params: Promise<{ path: string[] }>) {
  ...
  const upstreamUrl = new URL(`${upstreamBase}/api/${path.join("/")}`);
  upstreamUrl.search = new URL(req.url).search;

  const headers = new Headers(req.headers);
  // Let upstream set response encoding/content-type; don't lie about host.
  headers.delete("host");
