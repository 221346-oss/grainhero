...
<AreaChart data={areaData}>
  ...
  <Area
    ...
    stackId="1"
    stroke="hsl(38, 92%, 50%)"
    fill={`url(#${neonPatternId("hsl(38, 92%, 50%)")})`}
    fillOpacity={1}
    strokeWidth={1.5}
  />
  <Area
    ...
    fill={`url(#${neonPatternId("hsl(142, 70%, 40%)")})`}
    fillOpacity={1}
    strokeWidth={1.5}
  />
</AreaChart>
...
export function SeverityBadge({ severity }: { severity: keyof typeof severityConfig }) {
  ...
  return (
    <span
      className={cn("inline-flex items-center gap-1.5 text-[12px] font-medium", config.className)}
    >
      <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
        ...
        <rect
          ...
          height="10"
          rx="0.5"
          opacity={on(severity === "high" || severity === "critical")}
        />
      </svg>
      {config.label}
    </span>
    ...
    <thead>
      <tr className="border-b border-border bg-muted/30">
        {["ID", "Title", "Status", "Priority", "Created"].map((h) => (
          <th key={h} className="text-left font-medium text-muted-foreground px-3 py-2">
            {h}
          </th>
        ))}
      </tr>
