...
function sevBadge(severity) {
  switch (severity) {
    case "critical": return "bg-red-100 text-red-800";    // Red
    case "error":    return "bg-orange-100 text-orange-800"; // Orange
    case "warning":  return "bg-amber-100 text-amber-800";   // Amber
    default:         return "bg-slate-100 text-slate-700";   // Gray
  }
}
```
...
const mrr = activeSubs.reduce((sum, sub) => 
  sum + (Number(sub.monthly_price) || 0), 0
);
```
...
**Input:**
```typescript
{ targetAdminId: string }  // UUID of admin to impersonate
```
