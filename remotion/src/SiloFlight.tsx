...
const Shot: React.FC<{
  ...
  const opacity = interpolate(local, [-fi, 0, duration - fo, duration], [0, 1, 1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: ease,
  });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0d09" }}>
      ...
      <Shot
        ...
        duration={120}
        fadeIn={0}
        cam={{ scale: [1.02, 1.46], y: [0, -60] }}
      />

      {/* 2. ROOF + HATCH */}
      <Shot
        src="plates/02-hatch.jpg"
        from={112}
        duration={58}
        cam={{ scale: [1.05, 2.6], rot: [0, 7] }}
      />

      {/* 3. HEADSPACE */}
      <Shot
        src="plates/03-headspace.jpg"
        from={164}
        duration={110}
        cam={{ scale: [1.6, 1.04], y: [-90, 40] }}
      >
        <Pin
          from={190}
          duration={70}
        ...
        <Pin
          ...
          name="BME680"
          value="VOC RISING"
          note="gas + pressure trend"
        />
      </Shot>

      {/* 4. GRAIN SURFACE */}
      <Shot
        src="plates/05-grain-surface.jpg"
        from={268}
        duration={84}
        cam={{ scale: [1.26, 1.02], y: [-46, 26] }}
      >
        <Pin
          from={286}
          duration={58}
      ...
      {/* 5. WALL PROBES */}
      <Shot
        src="plates/04-wall-probes.jpg"
        from={346}
        duration={94}
        cam={{ scale: [1.3, 1.04], x: [90, -60] }}
      >
        <Pin
          from={364}
          duration={66}
      ...
      {/* 6. AERATION FLOOR */}
      <Shot
        src="plates/06-aeration-floor.jpg"
        from={434}
        duration={78}
        cam={{ scale: [1.06, 1.34], y: [36, -28] }}
      >
        <Pin
          from={452}
          duration={52}
          ...
          name="AERATION FLOOR"
          value="AIR MOVES"
          note="grain stays put"
        />
      </Shot>

      {/* 7. FAN */}
      <Shot
        src="plates/07-fan.jpg"
        from={506}
        duration={66}
        cam={{ scale: [1.24, 1.02], x: [-70, 40] }}
      >
        <Pin
          from={522}
          duration={44}
      ...
      {/* 8. CONTROLLER */}
      <Shot
        src="plates/09-controller.jpg"
        from={566}
        duration={66}
        cam={{ scale: [1.02, 1.28], x: [30, -40] }}
      >
        <Pin
          from={582}
          duration={44}
          ...
          name="ESP32-S3"
          value="LIVE"
          note="edge node · OLED + alerts"
        />
      </Shot>

      {/* 9. EXIT */}
      <Shot
        src="plates/08-exit.jpg"
        from={626}
        duration={66}
        cam={{ scale: [1.05, 1.95], rot: [4, -2] }}
      />
