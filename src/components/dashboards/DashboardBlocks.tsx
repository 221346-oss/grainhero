...
export function DashboardSiloCards({ range }: { range?: string } = {}) {
  ...
  return (
    <Card className="border-border/60 shadow-sm">
      <CardHeaderLink to="/grain-operations" search={{ tab: "silos" }} title="Silo Status" count={rows.length} />
      <CardContent className="p-3 pt-0">
        {rows.length === 0 ? (
          <p className="text-xs text-muted-foreground py-4 text-center">No silos yet</p>
        ) : (
          <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
            {rows.map((s) => {
              ...
              const pct = cap ? Math.round((occ / cap) * 100) : 0;
              const bar = pct >= 90 ? "bg-red-500" : pct >= 70 ? "bg-amber-500" : "bg-emerald-500";
              const badge = siloStatusBadge(pct, s.status);
              return (
                <div key={s.id} className="shrink-0 w-48 rounded-lg border border-border/60 bg-card p-2.5 space-y-2">
                  <div className="flex items-center justify-between gap-1">
                    <p className="text-xs font-semibold truncate">{s.name}</p>
                    <Badge className={badge.cls} variant="outline">{badge.label}</Badge>
                  </div>
