    ...
    <section className="rounded-xl border border-border bg-card/60 p-3 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-0.5 mt-1.5">
          {Array.from({ length: 10 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`h-1.5 flex-1 rounded-sm transition-all duration-500 ${bgClass}`}
      ...
      <div className="grid gap-2 md:grid-cols-[1.2fr_1fr]">
        ...
        >
          ...
          <div className="mt-2">
            <div
              className={`flex items-center gap-1 text-[11px] font-semibold ${positive ? "text-success" : "text-severity-critical"}`}
            >
                    ...
                    <defs>
                      <linearGradient id="platformRevSparkGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop
                          offset="0%"
                          stopColor={
                            positive ? "hsl(var(--success))" : "hsl(var(--severity-critical))"
                          }
                          stopOpacity={0.3}
                        />
                        <stop
