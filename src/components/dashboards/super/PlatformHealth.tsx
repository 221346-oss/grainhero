...
const tones = {
  success: {
    text: "text-success",
    stroke: "var(--success)",
    glow: "color-mix(in oklab, var(--success) 45%, transparent)",
  },
  warning: {
    text: "text-warning",
    stroke: "var(--warning)",
    glow: "color-mix(in oklab, var(--warning) 45%, transparent)",
  },
  critical: {
    text: "text-severity-critical",
    stroke: "var(--severity-critical)",
    glow: "color-mix(in oklab, var(--severity-critical) 45%, transparent)",
  },
};
...
export function PlatformHealth({
  ...
  const ticks = trend.length
    ? [
        trend[0],
        trend[Math.floor(trend.length / 3)],
        trend[Math.floor((trend.length * 2) / 3)],
  ...
  return (
    <Panel className="p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <span
          className={`inline-flex items-center gap-1.5 rounded-full bg-muted/40 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider ${tone.text}`}
        >
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0 flex-1">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trend} margin={{ top: 6, right: 4, left: 0, bottom: 0 }}>
                ...
                <YAxis
                  hide
                  domain={[
                    (dataMin: number) => Math.max(0, dataMin - 12),
                    (dataMax: number) => Math.min(100, dataMax + 12),
                  ]}
                />
                <Tooltip
          ...
          <div className="mt-1 flex justify-between text-[10px] tabular-nums text-muted-foreground/60">
            {ticks.map((t, i) => (
              <span key={`${t.date}-${i}`}>{shortDate(t.date)}</span>
            ))}
          </div>
        </div>
      </div>
