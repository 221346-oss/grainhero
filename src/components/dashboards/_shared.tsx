...
export function PageHeader({ title, subtitle, badge }: { title: string; subtitle?: string; badge?: string }) {
  return (
    <div className="mb-6 sm:mb-8 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
      <div className="min-w-0">
        <h1 className="truncate text-xl sm:text-2xl md:text-3xl font-black tracking-tight text-foreground">{title}</h1>
        {subtitle && <p className="mt-1 text-sm text-muted-foreground line-clamp-2">{subtitle}</p>}
      </div>
      {badge && (
        <Badge className="shrink-0 border-0 text-[--fusion-ink] font-bold" style={{ background: "var(--gradient-fusion)" }}>
          {badge}
        </Badge>
      )}
    </div>
