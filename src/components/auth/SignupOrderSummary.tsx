  ...
  return (
    <div className="rounded-lg border border-emerald-200 bg-emerald-50/80 p-4 space-y-3 text-sm">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="font-semibold text-slate-900 shrink-0">Rs. {totals.monthlyPrice.toLocaleString()}/mo</span>
      </div>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2 min-w-0">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600">Rs. {totals.iotUnit.toLocaleString()} per sensor (one-time)</p>
          </div>
        </div>
        <span className="font-semibold text-slate-900 shrink-0">Rs. {totals.iotTotal.toLocaleString()}</span>
      </div>
      ...
      {!paid && (
        <p className="text-[11px] text-slate-500">Includes first month + sensor setup. Then Rs. {totals.monthlyPrice.toLocaleString()}/mo.</p>
      )}
    </div>
  );
}
