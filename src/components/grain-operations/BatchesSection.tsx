...
const batchExportColumns: ExportColumn<Batch>[] = [
  ...
  {
    header: "Remaining (kg)",
    value: (b) => Math.max(0, Number(b.quantity_kg ?? 0) - Number(b.dispatched_quantity_kg ?? 0)),
  },
  {
    header: "Intake date",
...
function validateSpoilageForm(s: Spoilage, maxLossKg: number | null): SpoilageErrors {
  ...
  if (s.estimated_loss_kg.trim() !== "") {
    ...
    else if (maxLossKg != null && loss > maxLossKg)
      errors.estimated_loss_kg = `Can't exceed batch quantity (${maxLossKg.toLocaleString()} kg)`;
  }
...
export function BatchesSection({ initialStatus }: { initialStatus?: string } = {}) {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: u }) => setMyId(u.user?.id ?? null));
  }, []);

  const [q, setQ] = useState("");
  const [statusFilter, setStatusFilter] = useState<string>(
    initialStatus && (FILTER_STATUSES as readonly string[]).includes(initialStatus)
      ? initialStatus
      : "all",
  );
  ...
  const techniciansQ = useQuery({
    ...
    queryFn: () =>
      listTechFn() as Promise<Array<{ id: string; name: string | null; email: string | null }>>,
  ...
  }, [data, q, statusFilter, grainFilter, isManager]);
  ...
  const stats = useMemo(() => {
    ...
    const risky = rows.filter(
      (r) =>
        (r.risk_score ?? 0) >= 40 || r.spoilage_label === "Risky" || r.spoilage_label === "Spoiled",
    ).length;
  ...
  const spoilageMut = useMutation({
    mutationFn: (payload: { id: string; s: Spoilage }) =>
      spoilageFn({
        data: {
          id: payload.id,
          type: payload.s.type,
          ...
          estimated_loss_kg: payload.s.estimated_loss_kg
            ? Number(payload.s.estimated_loss_kg)
            : null,
    ...
    onSuccess: (row: any) => {
      ...
      if (softError) {
        toast.error(typeof softError === "string" ? softError : "Log failed");
        return;
      }
  ...
  function openEdit(b: Batch) {
    // Managers can only edit their own pending_approval batches
    // They cannot edit incoming batches from admin or outgoing batches
    if (isManager && b.status !== "pending_approval") {
      toast.error("You can only edit batches awaiting approval");
      return;
    }
    ...
    setForm({
      ...
      source_kind:
        ((b as { source_kind?: string }).source_kind as Form["source_kind"]) ?? "external",
      ...
      intake_temperature:
        b.intake_conditions?.temperature != null ? String(b.intake_conditions.temperature) : "",
      intake_humidity:
        b.intake_conditions?.humidity != null ? String(b.intake_conditions.humidity) : "",
      status: b.status,
      notes: b.notes ?? "",
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {form.id
                ...
                : "Batch ID and QR code are generated automatically on intake."}
            ...
            onSubmit={(e) => {
              ...
              if (form.source_kind === "anonymous") {
                if (!form.farmer_name.trim()) {
                  toast.error("Enter a walk-in name for anonymous source");
                  return;
                }
              ...
              if (!form.silo_id) {
                toast.error("Pick a silo");
                return;
              }
              if (!form.id && !form.assignedTechnicianId) {
                toast.error("Assign a technician for QC");
                return;
              }
              if (!form.grain_type) {
                toast.error("Pick a grain type");
                return;
              }
              ...
              if (!form.purchase_price_per_kg || Number(form.purchase_price_per_kg) <= 0) {
                toast.error("Purchase price / kg is required for cost tracking");
                return;
              }
              if (hasFormErrors) {
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm({ ...form, grain_type: v as GrainType })}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    formErrors.quantity_kg && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div className="sm:col-span-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Assign silo" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {!form.id && (
                <div className="sm:col-span-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setForm({ ...form, assignedTechnicianId: v })}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Pick a technician for QC" />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  {techniciansQ.data?.length === 0 && !techniciansQ.isLoading && (
                    <p className="text-xs text-rose-600 mt-1">
                      No technician is free right now — everyone already has a batch in progress.
                    </p>
                  )}
                  <p className="text-xs text-muted-foreground mt-1">
                    The batch starts in "Pending QC" — this technician inputs the QC values.
                  </p>
                </div>
              )}
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    formErrors.moisture_content && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    formErrors.protein_content && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    formErrors.test_weight && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    formErrors.purchase_price_per_kg && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm({ ...form, source_kind: v as Form["source_kind"], supplier_id: "" })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                ...
                {form.source_kind === "anonymous" ? (
                  ...
                  (() => {
                    ...
                    return (
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Pick from suppliers" />
                        </SelectTrigger>
                        <SelectContent>
                          {options.map((s) => (
                            <SelectItem key={s.id} value={s.id}>
                ...
                <Link
                  to="/suppliers"
                  className="text-[10px] text-muted-foreground hover:text-emerald-600 underline mt-1 inline-block"
                >
                  Manage suppliers →
                </Link>
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, farmer_contact: e.target.value })}
                  placeholder="Override contact"
                />
              </div>
              <div className="sm:col-span-2">
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    formErrors.intake_temperature && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    formErrors.intake_humidity && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              disabled={
                ...
                !form.quantity_kg ||
                !form.silo_id ||
                (!form.id && !form.assignedTechnicianId) ||
                hasFormErrors
              }
            >
              {saveMut.isPending ? (
            ...
            <>
              ...
              <div className="space-y-3 text-sm py-2">
                ...
                <Row label="Farmer">
                  ...
                  {selected.farmer_contact ? ` · ${selected.farmer_contact}` : ""}
                </Row>
                <Row label="Source">{selected.source_location ?? "—"}</Row>
                {selected.intake_date && (
                  <Row label="Intake">{new Date(selected.intake_date).toLocaleDateString()}</Row>
                )}
                {selected.moisture_content != null && (
                  <Row label="Moisture">{selected.moisture_content}%</Row>
                )}
                {selected.protein_content != null && (
                  <Row label="Protein">{selected.protein_content}%</Row>
                )}
                {selected.notes && (
                  <Row label="Notes">
        ...
        <DialogContent className="max-w-md max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-rose-600" /> Log spoilage event
            </DialogTitle>
          ...
          >
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Select
                  value={spoilage.type}
                  onValueChange={(v) => setSpoilage({ ...spoilage, type: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setSpoilage({ ...spoilage, severity: v as Spoilage["severity"] })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    spoilageErrors.estimated_loss_kg && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    spoilageErrors.temperature && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div>
                ...
                <Input
                  ...
                  className={cn(
                    spoilageErrors.humidity && "border-red-500 focus-visible:ring-red-500",
                  )}
                />
              ...
              <div className="col-span-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setSpoilage({ ...spoilage, description: e.target.value })}
                />
              </div>
              <div className="col-span-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setSpoilage({ ...spoilage, action_taken: e.target.value })}
                />
              </div>
            </div>
          </form>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setSpoilageOpen(false)}>
              Cancel
            </Button>
            <Button
              form="spoilage-form"
              type="submit"
              disabled={spoilageMut.isPending || hasSpoilageErrors}
            >
