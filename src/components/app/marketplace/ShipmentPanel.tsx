...
import { createShipment, appendShipmentEvent, markDelivered, getShipmentTracking } from "@/lib/dispatch.functions";
...
export function ShipmentPanel({
  ...
  const cMut = useMutation({
    mutationFn: () => createFn({ data: { orderId, courierKey: courier, trackingNumber: tracking || null } }),
    onSuccess: () => { toast.success("Shipment created"); invalidate(); },
    onError: (e) => toast.error((e as Error).message),
  });
  const aMut = useMutation({
    mutationFn: () => appendFn({ data: {
      shipmentId: data!.shipment!.id, code: nextStatus || "update",
      label: eventLabel || "Update",
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      setStatus: (nextStatus || undefined) as any,
    } }),
    onSuccess: () => { toast.success("Event added"); setEventLabel(""); setNextStatus(""); invalidate(); },
    onError: (e) => toast.error((e as Error).message),
  });
          ...
          <div className="space-y-2 rounded-md p-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              <div className="flex items-end">
                <Button className="w-full" disabled={!courier || cMut.isPending} onClick={() => cMut.mutate()}>Create shipment</Button>
              </div>
            </div>
          </div>
        )}
        {!ship && !canManage && <div className="text-sm text-muted-foreground">Waiting for the seller to dispatch.</div>}
        {ship && (
          <>
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <Badge variant="secondary">{ship.status}</Badge>
              <span>{ship.courier_label}</span>
              {ship.tracking_number && <span className="text-muted-foreground">#{ship.tracking_number}</span>}
              {ship.tracking_url && (
                <a href={ship.tracking_url} target="_blank" rel="noopener noreferrer" className="text-primary underline">
                  Track on courier
                </a>
              )}
            </div>
            ...
            {canManage && ship.status !== "delivered" && (
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 border-t pt-3">
                <Input placeholder="Event label" value={eventLabel} onChange={(e) => setEventLabel(e.target.value)} />
                ...
                <Button variant="outline" disabled={aMut.isPending || !eventLabel} onClick={() => aMut.mutate()}>Append event</Button>
                <Button disabled={dMut.isPending} onClick={() => dMut.mutate()}>Mark delivered</Button>
              </div>
            )}
          </>
        )}
