...
export function BuyerDisputeCard({ orderId }: { orderId: string }) {
  ...
  const [files, setFiles] = useState<Array<{ path: string; name: string; mime?: string; size?: number }>>([]);
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (files.length + selected.length > 10) { toast.error("Max 10 attachments"); return; }
    setUploading(true);
    try {
      ...
      for (const f of selected) {
        ...
        const { error } = await supabase.storage.from("dispute-attachments").upload(path, f, { contentType: f.type });
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { toast.success("Dispute submitted"); setOpen(false); setDescription(""); setCategory(""); setFiles([]); },
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <label className="inline-flex items-center gap-2 text-sm px-3 py-1.5 border rounded cursor-pointer hover:bg-muted">
            ...
            <input type="file" multiple className="hidden" accept="image/*,.pdf" disabled={uploading} onChange={handleUpload} />
          </label>
          {files.length > 0 && (
            <ul className="space-y-1">
              {files.map((f, i) => (
                <li key={f.path} className="flex items-center justify-between text-xs bg-muted/50 rounded px-2 py-1">
                  <span className="truncate">{f.name}</span>
                  <button type="button" className="text-muted-foreground hover:text-rose-600"
