...
import {
  ...
  listBatchEvents,
} from "@/lib/grain-batch-lifecycle.functions";
...
export function BatchLifecycleActions({
  ...
  const doTransition = useMutation({
    mutationFn: () =>
      transition({ data: { batchId, toState: toState as never, note: note || undefined } }),
    onSuccess: () => {
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Transition {batchLabel}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="text-sm">
              Current state: <span className="font-medium capitalize">{from || "…"}</span>
            </div>
            <div className="space-y-1">
              <Label>Next state</Label>
              <Select value={toState} onValueChange={setToState}>
                <SelectTrigger>
                  <SelectValue placeholder={next.length ? "Select" : "No transitions"} />
                </SelectTrigger>
                <SelectContent>
                  {next.map((s) => (
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => doTransition.mutate()}
              disabled={!toState || doTransition.isPending}
      ...
      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Timeline — {batchLabel}</DialogTitle>
          </DialogHeader>
          <div className="max-h-96 overflow-y-auto space-y-2">
            {(
              (events.data as { events?: Array<Record<string, unknown>> } | undefined)?.events ?? []
            ).map((e) => {
              ...
              return (
                <div key={ev.id as string} className="rounded-md p-2 text-sm">
                  ...
                  {ev.note ? (
                    <div className="text-xs text-muted-foreground mt-1">{String(ev.note)}</div>
                  ) : null}
                </div>
              );
            })}
