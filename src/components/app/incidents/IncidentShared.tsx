...
import { Clock, Archive, X, Search, CheckCircle2, Ban, Users } from "lucide-react";
...
export function IncidentTabNav({
  counts = {},
  basePath = "/platform/field-incidents",
}: {
  counts?: TabCounts;
  basePath?: string;
}) {
    ...
    <div className="flex flex-wrap items-center gap-1.5 mb-3">
      ...
      {search && (
        <button
          ...
          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] bg-muted border border-border hover:bg-muted/80"
        >
      ...
      {severity !== "all" && (
        <button
          ...
          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] bg-muted border border-border hover:bg-muted/80 capitalize"
        >
      ...
      <Card className="shadow-md border-primary/30">
        <CardContent className="p-5 space-y-4 text-sm">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge
              className={`border text-xs ${SEVERITY_COLOR[row.severity] ?? "bg-slate-100 text-slate-600 border-slate-200"}`}
            >
              {row.severity}
            </Badge>
            <Badge
              className={`border text-xs inline-flex items-center gap-1 ${STATUS_COLOR[row.status] ?? "bg-slate-100 text-slate-600 border-slate-200"}`}
            >
            ...
            <p className="text-xs bg-muted/40 rounded px-2 py-1.5">
              {reporterName ? (
                <>
                  ...
                  {reporterRole ? (
                    <span className="text-muted-foreground"> · {reporterRole}</span>
                  ) : null}
                </>
              ) : (
