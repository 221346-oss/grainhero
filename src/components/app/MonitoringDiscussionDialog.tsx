...
interface Props {
  ...
  incident: MonitoringIncidentItem | null;
...
export function MonitoringDiscussionDialog({ open, onOpenChange, incident, currentUserId }: Props) {
  const qc = useQueryClient();
  const listCommentsFn = useServerFn(listMonitoringIncidentComments);
  const addCommentFn = useServerFn(addMonitoringIncidentComment);

  const [message, setMessage] = useState("");
  const scrollRef = useRef<HTMLDivElement>(null);

  // Fetch comments for monitoring incidents
  const { data, isLoading } = useQuery({
    queryKey: ["monitoring-incident-comments", incident?.id],
  ...
  const responseData = data as
    | { comments: CommentRow[]; isParticipant: boolean }
    | CommentRow[]
    | undefined;
  const comments: CommentRow[] = Array.isArray(responseData)
    ? responseData
    : (responseData?.comments ?? []);
  const isParticipant = Array.isArray(responseData) ? true : (responseData?.isParticipant ?? false);
  ...
  const sendMut = useMutation({
    ...
    onSuccess: () => {
      setMessage("");
      qc.invalidateQueries({ queryKey: ["monitoring-incident-comments", incident?.id] });
    },
          ...
          <div className="flex-1 p-4 flex items-center justify-center">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                You're not authorized to view this discussion.
              </p>
            </div>
          </div>
        ) : (
