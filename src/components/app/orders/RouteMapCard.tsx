...
export function RouteMapCard({ originAddress, originLat, originLng, destAddress, destLat, destLng }: Props) {
  const hasCoords =
    typeof originLat === "number" && typeof originLng === "number" && typeof destLat === "number" && typeof destLng === "number";
      ...
      <div className="relative h-[240px] bg-gradient-to-br from-primary/5 via-muted/40 to-primary/10 dark:from-primary/10 dark:via-muted/20 dark:to-primary/20">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-30 dark:opacity-25" viewBox="0 0 400 240" preserveAspectRatio="none">
          <g stroke="currentColor" strokeWidth="0.6" className="text-muted-foreground">
            {[...Array(9)].map((_, i) => (<line key={`h${i}`} x1="0" y1={24 * (i + 1)} x2="400" y2={24 * (i + 1)} />))}
            {[...Array(15)].map((_, i) => (<line key={`v${i}`} x1={24 * (i + 1)} y1="0" x2={24 * (i + 1)} y2="240" />))}
          </g>
          {/* curved route line */}
          <path d="M 60 190 C 140 160, 200 120, 340 60" fill="none" stroke="hsl(var(--primary))" strokeWidth="3" strokeLinecap="round" strokeDasharray="6 6" />
        </svg>
        {/* origin marker */}
      ...
      <div className="p-3 space-y-2 text-xs">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <div className="text-muted-foreground uppercase tracking-wide font-semibold">Origin</div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <div className="text-muted-foreground uppercase tracking-wide font-semibold">Destination</div>
