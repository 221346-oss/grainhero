...
export function InstallationDrawer({ orderId, open, onOpenChange, canEdit }: Props) {
  ...
  const isAssignedTech =
    isTechnician &&
    !!(myProfile.data as any)?.id &&
    (myProfile.data as any)?.id === install?.technician_id;
  const canAdvanceAs = {
      ...
      saveFn({
        data: {
          ...
          patch: {
            ...
            scheduled_visit_at: form.scheduled_visit_at
              ? new Date(form.scheduled_visit_at).toISOString()
              : null,
  ...
  const mapProps = useMemo(
    () => ({
      ...
      destAddress:
        form.destination_address ?? install?.destination_address ?? order?.install_address,
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>
            Installation tracking {order?.id ? `· ${String(order.id).slice(0, 8)}` : ""}
          </SheetTitle>
        </SheetHeader>
        {q.isLoading ? (
          ...
          <div className="mt-4 space-y-6">
            ...
            {order && !order.admin_id && (
              <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                ...
                the admin completes payment and the order is claimed. Ask the admin to go to their
                install orders page and complete payment.
              </div>
            )}
            <InstallStageTracker
              stage={stage}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <label className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                        <CalIcon className="h-3 w-3" /> Scheduled visit
                      </label>
                      <Input
                        type="datetime-local"
                        value={
                          form.scheduled_visit_at
                            ? new Date(form.scheduled_visit_at).toISOString().slice(0, 16)
                            : ""
                        }
                        onChange={(e) => set("scheduled_visit_at", e.target.value)}
                      />
                    </div>
                  </div>
                  {adminId && (
            ...
            <div>
              ...
              {canEdit && (
                <div className="grid grid-cols-1 gap-2 mt-3">
                  <Field
                    label="Origin address (company HQ — editable in Platform settings)"
                    v={form.origin_address}
                    onChange={(v) => set("origin_address", v)}
                    full
                  />
                  <Field
                    label="Destination address (from buyer's install order)"
                    v={form.destination_address}
                    onChange={(v) => set("destination_address", v)}
                    full
                  />
                  <div className="text-[11px] text-muted-foreground">
                    Coordinates are captured from the buyer's map pin at checkout; no need to enter
                    lat/lng manually.
                  </div>
                </div>
              )}
            ...
            {canEdit && (
              <Button onClick={() => saveM.mutate()} disabled={saveM.isPending} className="w-full">
                ...
                {saveM.isPending ? "Saving…" : "Save installation details"}
              </Button>
            )}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                {canEdit && (
                  <Button
                    ...
                    variant="ghost"
                    onClick={() =>
                      setDevices((d) => [...d, { serial: "", model: "", status: "shipped" }])
                    }
                  >
              ...
              <div className="space-y-2">
                {devices.length === 0 && (
                  <div className="text-xs text-muted-foreground">No devices linked yet.</div>
                )}
                {devices.map((d, i) => (
                  <div key={i} className="grid grid-cols-[1fr_1fr_auto_auto] gap-2 items-center">
                    <Input
                      ...
                      onChange={(e) =>
                        setDevices((arr) =>
                          arr.map((x, idx) => (idx === i ? { ...x, serial: e.target.value } : x)),
                        )
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setDevices((arr) =>
                          arr.map((x, idx) => (idx === i ? { ...x, model: e.target.value } : x)),
                        )
                      }
                    />
                    <select
                      ...
                      onChange={(e) =>
                        setDevices((arr) =>
                          arr.map((x, idx) => (idx === i ? { ...x, status: e.target.value } : x)),
                        )
                      }
                    >
                      {DEVICE_STATUS.map((s) => (
              ...
              {canEdit && devices.length > 0 && (
                <Button
                  ...
                  variant="outline"
                  className="mt-2"
                  onClick={() => saveDevM.mutate()}
                  disabled={saveDevM.isPending}
                >
                  Save devices
                </Button>
              )}
            ...
            <div>
              ...
              <div className="space-y-2 mb-3">
                {(q.data?.events ?? []).length === 0 && (
                  <div className="text-xs text-muted-foreground">No visit events yet.</div>
                )}
              ...
              {canEdit && (
                <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={() => noteM.mutate()} disabled={!note.trim() || noteM.isPending}>
