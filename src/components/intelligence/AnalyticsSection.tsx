...
export function AnalyticsSection() {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Analytics is available to managers, admins and super admins.
          </CardDescription>
        </CardHeader>
      </Card>
    );
  ...
  return (
    <div className="space-y-6">
        ...
        <PlatformOverviewTable
          ...
          columns={[
            ...
            },
            {
              ...
              align: "right",
              render: (r) => (
                <span className={r.spoilageRate > 0.1 ? "text-red-600 font-medium" : ""}>
                  {(r.spoilageRate * 100).toFixed(1)}%
                </span>
              ),
            },
          ]}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <div className="text-xs uppercase text-slate-500 font-semibold">Total inventory</div>
              <div className="text-2xl font-bold text-slate-900">{fmtKg(t?.totalKg ?? 0)}</div>
              <div className="text-xs text-slate-500 mt-1">{t?.batches ?? 0} batches</div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-bold text-slate-900">
                {fmtMoney(t?.totalRevenue ?? 0)}
              </div>
              <div className="text-xs text-emerald-600 mt-1">
                {((t?.margin ?? 0) * 100).toFixed(1)}% margin
              </div>
            </div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <div className="text-xs uppercase text-slate-500 font-semibold">Capacity used</div>
              <div className="text-2xl font-bold text-slate-900">
                {((t?.utilization ?? 0) * 100).toFixed(0)}%
              </div>
              <Progress value={(t?.utilization ?? 0) * 100} className="h-1.5 mt-2" />
            </div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <div className="text-xs uppercase text-slate-500 font-semibold">Spoilage rate</div>
              <div className="text-2xl font-bold text-red-600">
                {((t?.spoilageRate ?? 0) * 100).toFixed(1)}%
              </div>
              <div className="text-xs text-slate-500 mt-1">
                {t?.spoiled ?? 0} affected · {t?.openAlerts ?? 0} open alerts
              </div>
            </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-xs uppercase text-slate-500 font-semibold">Avg temperature</div>
              <div className="text-xl font-bold">{(env?.avgTemp ?? 0).toFixed(1)}°C</div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-xs uppercase text-slate-500 font-semibold">Avg humidity</div>
              <div className="text-xl font-bold">{(env?.avgHum ?? 0).toFixed(1)}%</div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-xs uppercase text-slate-500 font-semibold">Avg moisture</div>
              <div className="text-xl font-bold">{(env?.avgMoist ?? 0).toFixed(1)}%</div>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Intake trend (30 days)</CardTitle>
          <CardDescription>Daily inventory received</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>By grain type</CardTitle>
            <CardDescription>Volume and revenue split</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
              ...
              return (
                <div key={g.grain}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="capitalize font-medium">{g.grain}</span>
                    <span className="text-slate-500">
                      {fmtKg(g.kg)} · {fmtMoney(g.revenue)}
                    </span>
                  </div>
            ...
            {byGrain.length === 0 && (
              <div className="text-sm text-slate-500 text-center py-6">No batches yet.</div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Distribution across priorities and batch states</CardDescription>
          </CardHeader>
