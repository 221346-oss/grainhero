...
export function ReportsSection() {
  ...
  const spoiled = filtered.batches.filter((b: any) => b.spoilage_label && String(b.spoilage_label).toLowerCase() !== "safe").length;
  const collected = filtered.invoices.reduce((s: number, i: any) => s + Number(i.amount_paid ?? 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card><CardContent className="p-4 flex justify-between items-center"><div><div className="text-xs uppercase text-muted-foreground font-semibold">Collected</div><div className="text-2xl font-bold">${collected.toLocaleString()}</div><div className="text-xs text-muted-foreground">{filtered.invoices.length} invoices</div></div><TrendingUp className="h-6 w-6 text-emerald-600" /></CardContent></Card>
        <Card><CardContent className="p-4 flex justify-between items-center"><div><div className="text-xs uppercase text-muted-foreground font-semibold">Alerts</div><div className="text-2xl font-bold">{filtered.alerts.length}</div><div className="text-xs text-muted-foreground">{alertsResolved} resolved · {spoiled} spoiled</div></div><AlertTriangle className="h-6 w-6 text-red-600" /></CardContent></Card>
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        {[
          { title: "Batches report", desc: "Grain batch inventory, quality and financials", rows: filtered.batches, file: "batches.csv" },
          { title: "Alerts report", desc: "Alerts triggered in the selected period", rows: filtered.alerts, file: "alerts.csv" },
          { title: "Invoices report", desc: "Buyer invoices and payment status", rows: filtered.invoices, file: "invoices.csv" },
          { title: "Silo utilization", desc: "Current capacity and stock across silos", rows: filtered.silos, file: "silos.csv" },
        ].map((r) => (
          <Card key={r.title}>
            ...
            <CardContent>
              <Button variant="outline" size="sm" disabled={r.rows.length === 0} onClick={() => download(r.file, toCsv(r.rows))}>
