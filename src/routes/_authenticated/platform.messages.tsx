  ...
  head: () => ({
    meta: [
      { title: "Platform · Messages — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Messages workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Messages — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Messages workspace in the Grain Hero platform.",
      },
...
function FlaggedMessagesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["platform-flagged-messages"],
    queryFn: () => list(),
  });
  const m = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["platform-flagged-messages"] });
    },
  ...
  return (
    <AdminPageShell
      ...
      subtitle="Review buyer/seller messages auto-flagged for moderation."
    >
      <Card>
              ...
              {data?.messages?.map((msg) => (
                <tr key={msg.id as string} className="border-b hover:bg-emerald-50/30 align-top">
                  <td className="p-3 font-mono text-xs">
                    {(msg.buyer_orders as { order_number?: string } | null)?.order_number ??
                      String(msg.order_id).slice(0, 8)}
                  </td>
                  <td>
                    <Badge variant="outline" className="capitalize">
                      {msg.sender_role as string}
                    </Badge>
                  </td>
                  ...
                  <td className="space-x-2">
                    <Button
                      size="sm"
                      onClick={() => m.mutate({ id: msg.id as string, action: "approve" })}
                    ...
                    <Button
                      size="sm"
                      variant="destructive"
                      onClick={() => m.mutate({ id: msg.id as string, action: "hide" })}
                      disabled={m.isPending}
                    >
                      Hide
                    </Button>
