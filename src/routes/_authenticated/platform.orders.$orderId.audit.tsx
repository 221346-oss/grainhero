  ...
  head: () => ({
    meta: [
      { title: "Platform · Orders · OrderId · Audit — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Orders · OrderId · Audit workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Orders · OrderId · Audit — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Orders · OrderId · Audit workspace in the Grain Hero platform.",
      },
...
function OrderAuditPage() {
  ...
  const [source, setSource] = useState<"all" | "shipment_event" | "order_event" | "activity_log">(
    "all",
  );
      ...
      actions={
        <Link to="/platform/orders/$orderId" params={{ orderId }}>
          <Button variant="outline" size="sm">
            <ArrowLeft className="h-3 w-3 mr-1" /> Back to order
          </Button>
        </Link>
      }
    >
          ...
          <CardTitle className="text-sm flex flex-wrap items-center gap-3">
            ...
            {data && (
              <span className="ml-auto text-xs text-muted-foreground">
                {data.counts.shipment_event} shipment · {data.counts.order_event} order ·{" "}
                {data.counts.activity_log} log
              </span>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="space-y-2">
            {data?.entries.map((e) => (
              <div
                key={e.id}
                className="border-l-2 border-emerald-500 pl-3 py-2 hover:bg-emerald-50/30 rounded-r transition-colors"
              >
