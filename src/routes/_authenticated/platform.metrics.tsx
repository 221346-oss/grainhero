...
function MetricRegistryPage() {
  ...
  const listQ = useQuery({ queryKey: ["metric-registry"], queryFn: () => fetchList({ data: { onlyMine: false, active: "all" } }) });
  const logQ = useQuery({ queryKey: ["refresh-log"], queryFn: () => fetchLog({ data: { limit: 20 } }) });
  ...
  const saveM = useMutation({
    ...
    onSuccess: () => { toast.success("Saved"); qc.invalidateQueries({ queryKey: ["metric-registry"] }); setEditing(null); },
  ...
  const removeM = useMutation({
    ...
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["metric-registry"] }); },
  });
  const refreshM = useMutation({
    mutationFn: () => refresh({ data: { scope: "all" } }),
    onSuccess: () => { toast.success("Warehouse refresh queued"); qc.invalidateQueries({ queryKey: ["refresh-log"] }); },
    onError: (e: Error) => toast.error(e.message),
  });
    ...
    >
          ...
          <tbody>
            {metrics.map((m) => (
              <tr key={m.id} className="border-t hover:bg-emerald-50/40">
                ...
                <td className="px-4 py-2">
                  <Switch checked={m.active} onCheckedChange={(v) => toggleM.mutate({ id: m.id, active: v })} />
                </td>
                <td className="px-4 py-2 text-right space-x-1">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => { if (confirm("Delete metric?")) removeM.mutate(m.id); }}>
                    Delete
                  </Button>
                </td>
              </tr>
      ...
      {preview_ && (
        <AdminDataCard title="Preview result">
          <pre className="p-4 text-xs overflow-auto bg-slate-950 text-emerald-200 rounded-b-lg">{preview_}</pre>
        </AdminDataCard>
      )}
...
function MetricEditorSheet({
  metric, onClose, onSave, saving,
}: { metric: Metric | null; onClose: () => void; onSave: (m: Metric) => void; saving: boolean }) {
  ...
  const toggleRole = (r: string) => {
    ...
    upd({ allowed_roles: has ? form.allowed_roles.filter((x: string) => x !== r) : [...form.allowed_roles, r] });
  };

  return (
    <Sheet open={!!metric} onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="sm:max-w-2xl overflow-y-auto">
        <SheetHeader><SheetTitle>{form.id ? "Edit" : "New"} metric</SheetTitle></SheetHeader>
        <div className="space-y-4 mt-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Key</Label>
              <Input value={form.key} onChange={(e) => upd({ key: e.target.value })} placeholder="mrr_total" disabled={!!form.id} />
            </div>
            <div>
              <Label>Label</Label>
              <Input value={form.label} onChange={(e) => upd({ label: e.target.value })} placeholder="MRR" />
            </div>
          </div>
          <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <Label>Unit</Label>
              <Input value={form.unit ?? ""} onChange={(e) => upd({ unit: e.target.value })} placeholder="USD" />
            </div>
            <div>
              <Label>Chart hint</Label>
              <Select value={form.chart_hint} onValueChange={(v) => upd({ chart_hint: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{HINTS.map((f) => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
          <div>
