  ...
  head: () => ({
    meta: [
      { title: "Admins · AdminId — Grain Hero" },
      {
        name: "description",
        content:
          "Admins · AdminId workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Admins · AdminId — Grain Hero" },
      {
        property: "og:description",
        content: "Admins · AdminId workspace in the Grain Hero platform.",
      },
...
function AdminProfilePage() {
  ...
  const profileQ = useQuery({
    ...
    queryFn: () => getProfile({ data: { adminId } }),
  });
  const freqQ = useQuery({
    queryKey: ["admin-frequency", adminId, source],
    queryFn: () => getFreq({ data: { adminId, source } }),
  });
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
        ...
        <CardContent className="p-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-muted-foreground flex items-center gap-4 flex-wrap mt-1">
              ...
              {p.business_type && (
                <span className="inline-flex items-center gap-1">
                  <Building2 className="h-3.5 w-3.5" /> {p.business_type}
                </span>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2">
            {p.suspended ? (
              <Badge className="bg-red-100 text-red-800 dark:bg-red-500/15 dark:text-red-300 border-transparent">
                Suspended
              </Badge>
            ) : (
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
        ...
        <KpiTile
          ...
          accent={stats.openAlerts > 0 ? "text-red-600 dark:text-red-400" : undefined}
        />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-1">
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Contact & plan</CardTitle>
          </CardHeader>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            ...
            <Tabs value={source} onValueChange={(v) => setSource(v as any)}>
              <TabsList className="h-8">
                <TabsTrigger value="batches" className="text-xs px-3">
                  Batches
                </TabsTrigger>
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground mt-2">
              Last 6 months of {source === "batches" ? "grain batches" : "hardware orders"} created
              by this admin.
            </div>
          </CardContent>
        </Card>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          <EditForm
            profile={p}
            onSubmit={(patch) => updateM.mutate(patch)}
...
function EditForm({
  ...
  profile: any;
  onSubmit: (p: { name?: string; phone?: string; notes?: string }) => void;
  pending: boolean;
}) {
