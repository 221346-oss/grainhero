...
import { listPushDeliveries, listRegisteredDevices, sendTestPush } from "@/lib/mobile-push-diagnostics.functions";
...
function PushDiagnosticsPage() {
  ...
  const { data: dels } = useQuery({ queryKey: ["mobile-push-deliveries"], queryFn: () => loadDel() });
  ...
  return (
    ...
    >
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>Send test push</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <Field label="Device ID">
              <Input value={deviceId} onChange={(e) => setDeviceId(e.target.value)} placeholder="uuid from devices list" />
            </Field>
            <Field label="Title"><Input value={title} onChange={(e) => setTitle(e.target.value)} /></Field>
            <Field label="Body"><Input value={body} onChange={(e) => setBody(e.target.value)} /></Field>
            <Button onClick={() => testMut.mutate()} disabled={!deviceId || testMut.isPending}>Send test</Button>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-2 max-h-96 overflow-y-auto">
            ...
            {devices.map((d) => (
              <div key={d.id as string} className="rounded-md p-2 text-xs">
                ...
                <div className="text-muted-foreground mt-1">seen: {d.last_seen_at ? new Date(d.last_seen_at as string).toLocaleString() : "—"}</div>
                {d.last_push_error ? <div className="text-red-600">err: {String(d.last_push_error)}</div> : null}
                <button
                  type="button"
        ...
        <Card className="xl:col-span-2">
          <CardHeader><CardTitle>Recent push deliveries (last 100)</CardTitle></CardHeader>
          <CardContent>
            <div className="max-h-96 overflow-y-auto text-xs">
              ...
              {deliveries.map((d) => (
                <div key={d.id as string} className="flex items-center justify-between border-b py-1.5">
                  <span>{new Date(d.created_at as string).toLocaleString()}</span>
                  <Badge variant={d.status === "sent" ? "secondary" : d.status === "failed" ? "destructive" : "outline"}>
                    {String(d.status)}
                  </Badge>
                  <span className="text-muted-foreground truncate max-w-[40%]">{String(d.error ?? d.provider_message_id ?? "")}</span>
                </div>
              ))}
            </div>
          </CardContent>
