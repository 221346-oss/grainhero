  ...
  head: () => ({
    meta: [
      { title: "Platform · Security — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Security workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Security — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Security workspace in the Grain Hero platform.",
      },
...
function PlatformSecurityPage() {
    ...
    return (
      <AdminPageShell title="Security" subtitle="Access restricted">
        <div className="p-8 text-center text-sm text-muted-foreground">
          Security workspace is available to admins and super admins.
        </div>
      </AdminPageShell>
    );
  }
  ...
  if (roleQ.isLoading)
    return (
      <AdminPageShell title="Security" subtitle="Loading...">
    ...
    >
      ...
      <div className="grid gap-px bg-border rounded-md overflow-hidden grid-cols-2 sm:grid-cols-5">
        ...
        <div className="bg-background px-3 py-3">
          ...
          <span className="text-xl font-bold tabular-nums text-severity-critical">
            {blockedCount}
          </span>
        </div>
        <div className="bg-background px-3 py-3">
      ...
      <HairlineGrid cols="grid-cols-1 lg:grid-cols-2">
        ...
        <NeonPanel
          index="01"
          title="User access"
          subtitle="Roles and blocked accounts - manage user access"
        >
          <div className="rounded-md overflow-hidden">
            <div className="divide-y divide-border/40 max-h-[500px] overflow-y-auto">
              {allUsers.map((u: any) => (
                <div
                  key={u.id}
                  className="p-3 flex items-center justify-between text-sm gap-3 hover:bg-muted/30 transition-colors"
                >
        ...
        <NeonPanel
          ...
          subtitle="Recent warnings and errors from the audit log"
        >
          <div className="rounded-md overflow-hidden">
              ...
              {logs.map((l: any) => (
                <div key={l.id} className="p-3 text-sm hover:bg-muted/30 transition-colors">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge className={sevBadge(l.severity) + " text-[10px] uppercase border"}>
                      {l.severity ?? "info"}
                    </Badge>
                  ...
                  {l.message && (
                    <div className="text-xs text-muted-foreground mt-1">{l.message}</div>
                  )}
