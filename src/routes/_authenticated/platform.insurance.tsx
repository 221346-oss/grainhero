  ...
  head: () => ({
    meta: [
      { title: "Platform · Insurance — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Insurance workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Insurance — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Insurance workspace in the Grain Hero platform.",
      },
...
function PlatformInsurancePage() {
  ...
  const tiles = [
    ...
    {
      label: "Total premium",
      value: `${kpis.currency} ${kpis.totalPremium.toLocaleString()}`,
      icon: DollarSign,
    },
    {
      label: "Total payout",
      value: `${kpis.currency} ${kpis.totalPayout.toLocaleString()}`,
      icon: TrendingDown,
    },
      ...
      <div className="space-y-6 max-w-[1400px]">
        ...
        <HairlineGrid cols="grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          {tiles.map((t) => (
            <div
              key={t.label}
              className="bg-background p-4 space-y-2 transition-colors hover:bg-muted/20"
            >
        ...
        <div className="flex justify-end gap-4">
          <Link
            to="/platform/insurance/webhooks"
...
function AnalyticsTab() {
  ...
  if (isLoading || !data)
    return <div className="p-6 text-sm text-muted-foreground rounded-md">Loading analytics…</div>;
  return (
            ...
            <table className="w-full text-[13px] min-w-[600px]">
              ...
              <tbody>
                ...
                {!data.products.length && (
                  <tr>
                    <td colSpan={5} className="text-center text-muted-foreground px-3 py-6">
...
function ClaimsQueue() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      listFn({ data: { scope: "queue" as const, status: status === "all" ? undefined : status } }),
  });
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <Select value={status} onValueChange={setStatus}>
          ...
          <SelectContent>
            {["all", "submitted", "under_review", "approved", "rejected", "paid", "cancelled"].map(
              (s) => (
      ...
      <MobileTableWrapper>
        <table className="w-full text-[13px] min-w-[700px]">
          ...
          <tbody>
            {(data?.claims ?? []).map((c: Record<string, unknown>) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 uppercase tracking-wide text-muted-foreground text-[11px]">
                    {c.claim_type as string}
                  </td>
                  <td className="px-3 py-2 tabular-nums">
                    {money(c.requested_payout_cents as number, cur)}
                  </td>
                  <td className="px-3 py-2 tabular-nums">
                    {c.approved_payout_cents ? money(c.approved_payout_cents as number, cur) : "—"}
                  </td>
                  <td className="px-3 py-2">
                    <StatusBadge
                      status={CLAIM_STATUS_TONE[c.status as string] ?? "neutral"}
                      label={(c.status as string).replace("_", " ")}
                    />
                  </td>
                  <td className="px-3 py-2">
                    <Button size="sm" variant="outline" onClick={() => setOpenId(c.id as string)}>
                      Review
                    </Button>
                  </td>
                </tr>
            ...
            {!(data?.claims ?? []).length && (
              <tr>
                <td colSpan={7} className="text-center text-muted-foreground px-3 py-8">
                  No claims to review.
                </td>
              </tr>
            )}
      ...
      {openId && (
        <ClaimReviewSheet
          ...
          onClose={() => {
            ...
            qc.invalidateQueries({ queryKey: ["ins-kpis"] });
          }}
        />
      )}
    </div>
...
function ClaimReviewSheet({ claimId, onClose }: { claimId: string; onClose: () => void }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => timelineFn({ data: { claim_id: claimId } }),
  });
    ...
    mutationFn: async (decision: "under_review" | "approved" | "rejected" | "paid") =>
      moderateFn({
        data: {
          id: claimId,
          decision,
          approved_payout_cents:
            decision === "approved" && approved ? Math.round(Number(approved) * 100) : undefined,
  ...
  return (
    <Sheet open onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Claim review</SheetTitle>
        </SheetHeader>
        {!claim ? (
          ...
          <div className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <StatusBadge
                  status={CLAIM_STATUS_TONE[claim.status] ?? "neutral"}
                  label={claim.status}
                />
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Loss</div>
                <div className="tabular-nums">{money(claim.loss_amount_cents, cur)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Requested</div>
                <div className="tabular-nums">{money(claim.requested_payout_cents, cur)}</div>
              </div>
            </div>
            {claim.narrative && (
            ...
            <div>
              <div className="text-xs text-muted-foreground mb-1">
                Evidence ({data?.attachments.length ?? 0})
              </div>
              <ul className="text-xs space-y-1">
                {(data?.attachments ?? []).map((a: Record<string, unknown>) => (
                  <li key={a.id as string} className="truncate">
                    {a.file_path as string}
                  </li>
                ))}
                {!data?.attachments.length && (
                  <li className="text-muted-foreground">No attachments</li>
                )}
              </ul>
            </div>
            <div>
              ...
              <ol className="space-y-1 text-xs">
                {(data?.events ?? []).map((e: Record<string, unknown>) => (
                  <li
                    key={e.id as string}
                    className="flex justify-between border-l-2 border-emerald-500/40 pl-2"
                  >
            ...
            <div className="space-y-3 rounded-md p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Input
                  placeholder={`Approved payout (${cur})`}
                  value={approved}
              ...
              <Textarea
                placeholder="Decision reason (shown to tenant)"
                value={reason}
...
function ProductsTab() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    ...
    name: "",
    coverage_type: "batch",
    base_premium_bps: "100",
    deductible_bps: "0",
    currency: "USD",
  });
  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ["ins-products"] });
  };
    ...
    <div className="space-y-3">
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="sm:max-w-md overflow-y-auto">
          <SheetHeader>
            <SheetTitle>New insurance product</SheetTitle>
          </SheetHeader>
          <div className="mt-6 grid grid-cols-2 gap-3">
            ...
            <Input
              placeholder="Code (e.g. BATCH-STD)"
              value={form.code}
            ...
            />
            <Input
              placeholder="Premium (bps)"
              type="number"
              value={form.base_premium_bps}
              onChange={(e) => setForm({ ...form, base_premium_bps: e.target.value })}
            />
            <Input
              placeholder="Deductible (bps)"
              type="number"
              value={form.deductible_bps}
              onChange={(e) => setForm({ ...form, deductible_bps: e.target.value })}
            />
            <Input
      ...
      <MobileTableWrapper>
        <table className="w-full text-[13px] min-w-[800px]">
          ...
          <tbody>
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    {p.active ? (
                      ...
                      <StatusBadge status="neutral" label="Off" />
                    )}
                  </td>
                  <td className="px-3 py-2">
                    <Button
                      ...
                      variant="ghost"
                      onClick={async () => {
                        await del({ data: { id: p.id as string } });
                        invalidate();
                      }}
                    >
            ...
            {!(data?.products ?? []).length && (
              <tr>
                <td colSpan={8} className="text-center text-muted-foreground px-3 py-8">
                  No products yet.
                </td>
              </tr>
            )}
    ...
    <div className="space-y-3">
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="sm:max-w-sm">
          <SheetHeader>
            <SheetTitle>New insurance carrier</SheetTitle>
          </SheetHeader>
        ...
        <table className="w-full text-[13px] min-w-[500px]">
          ...
          <tbody>
              ...
              >
                ...
                <td className="px-3 py-2">
                  <Button
                    ...
                    variant="ghost"
                    onClick={async () => {
                      await del({ data: { id: c.id as string } });
                      invalidate();
                    }}
                  >
                    <Trash2 className="h-4 w-4" />
            ...
            {!(data?.carriers ?? []).length && (
              <tr>
                <td colSpan={5} className="text-center text-muted-foreground px-3 py-8">
