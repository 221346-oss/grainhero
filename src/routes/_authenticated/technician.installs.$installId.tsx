...
function TechnicianInstallDetailPage() {
  ...
  const setStatus = useMutation({
    mutationFn: (v: { status: "scheduled"|"en_route"|"onsite"|"completed"|"blocked"; blockerNote?: string }) =>
  ...
  const addEvent = useMutation({
    ...
    onSuccess: () => { toast.success("Event logged"); invalidate(); },
    onError: (e) => toast.error(String(e)),
  });
  ...
  if (isLoading || !data) return <div className="p-6"><div className="h-32 rounded-xl bg-muted animate-pulse" /></div>;
  const { install, devices, events, silos, buyer } = data;
  const status = install.status as string;

  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-6">
      <Link to="/technician/installs" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> My installs
      </Link>

      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Install {install.id.slice(0, 8)}</h1>
          {install.scheduled_for && <p className="text-sm text-muted-foreground">Scheduled {new Date(install.scheduled_for).toLocaleString()}</p>}
        </div>
      ...
      <div className="flex items-center gap-1 overflow-x-auto">
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs capitalize border transition-colors ${
                done ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-slate-700 border-slate-200 hover:border-emerald-400"
              }`}
            >
              {idx + 1}. {s.replace("_", " ")}
            </button>
        ...
        <Button size="sm" variant="outline" className="ml-2 text-rose-700"
          onClick={() => {
            const note = prompt("Blocker reason?") || "";
            if (note.trim().length >= 3) setStatus.mutate({ status: "blocked", blockerNote: note.trim() });
          }}>
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><HardHat className="h-4 w-4" /> Log a visit event</CardTitle></CardHeader>
        <CardContent><EventForm onSubmit={(v) => addEvent.mutate(v)} /></CardContent>
      </Card>
          ...
          {events.length === 0 ? <div className="text-muted-foreground">No events yet.</div> :
            events.map((e) => (
              <div key={e.id as string} className="py-1.5 border-b last:border-0">
                ...
                <div className="text-xs text-muted-foreground">{new Date(e.created_at as string).toLocaleString()}</div>
              </div>
            ))}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {devices.length === 0 ? <div className="text-sm text-muted-foreground">No devices on this order.</div> :
            devices.map((d) => (
              <div key={d.id as string} className="rounded border p-3">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-medium">{d.device_type ?? "device"}</div>
                  {d.commissioned_at ? (
                    <Badge className="bg-emerald-100 text-emerald-800"><CheckCircle2 className="h-3.5 w-3.5 mr-1" />Commissioned</Badge>
                  ) : (
                ...
                {d.commissioned_at ? (
                  ...
                  <CommissionForm
                    silos={silos}
                    onSubmit={(v) => commission.mutate({ orderDeviceId: d.id as string, serialNumber: v.serial, siloId: v.siloId, deviceType: (d.device_type as string) ?? "sensor" })}
                  />
                )}
              </div>
            ))}
...
function EventForm({ onSubmit }: { onSubmit: (v: { eventType: "arrived"|"inspection"|"install"|"test"|"handover"|"issue"; note?: string }) => void }) {
  const [type, setType] = useState<"arrived"|"inspection"|"install"|"test"|"handover"|"issue">("arrived");
  ...
  return (
    <div className="grid gap-3 md:grid-cols-[180px_1fr_auto]">
      ...
      <Textarea rows={1} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (optional)" />
      <Button onClick={() => { onSubmit({ eventType: type, note: note.trim() || undefined }); setNote(""); }}>Log</Button>
    </div>
  );
}

function CommissionForm({ silos, onSubmit }: { silos: Array<Record<string, unknown>>; onSubmit: (v: { serial: string; siloId: string }) => void }) {
  const [serial, setSerial] = useState("");
  const [siloId, setSiloId] = useState("");
  return (
    <div className="grid gap-2 md:grid-cols-[1fr_1fr_auto] mt-2">
      <div className="space-y-1"><Label className="text-xs">Serial number</Label><Input value={serial} onChange={(e) => setSerial(e.target.value)} /></div>
      <div className="space-y-1"><Label className="text-xs">Silo</Label>
        <Select value={siloId} onValueChange={setSiloId}>
          <SelectTrigger><SelectValue placeholder="Pick silo" /></SelectTrigger>
          <SelectContent>
            {silos.map((s) => <SelectItem key={s.id as string} value={s.id as string}>{s.name as string}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      <div className="flex items-end">
        <Button disabled={serial.length < 3 || !siloId} onClick={() => onSubmit({ serial: serial.trim(), siloId })}>Commission</Button>
      </div>
    </div>
  );
}
