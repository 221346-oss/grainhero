...
function OrderDetail() {
  ...
  const cancel = useMutation({
    ...
    onSuccess: () => { toast.success("Order cancelled"); qc.invalidateQueries({ queryKey: ["my-order", orderId] }); },
    onError: (e) => toast.error((e as Error).message),
  });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <Link to="/buyer/orders" className="text-sm text-muted-foreground hover:text-foreground">← All orders</Link>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-sm text-muted-foreground">
            {Number(o.quantity_kg).toLocaleString()} kg × {o.currency} {Number(o.unit_price).toFixed(2)}
          </div>
        </div>
        <Badge variant="outline" className="capitalize text-base">{o.status}</Badge>
      </div>
      ...
      <Card>
        <CardContent className="p-5 space-y-3">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold">{o.currency} {Number(o.subtotal).toFixed(2)}</span>
          </div>
          {o.paid_at && (
          ...
          {o.dispatched_at && <div className="text-sm text-muted-foreground">Dispatched {new Date(o.dispatched_at).toLocaleString()}</div>}
          {o.completed_at && <div className="text-sm text-muted-foreground">Completed {new Date(o.completed_at).toLocaleString()}</div>}
