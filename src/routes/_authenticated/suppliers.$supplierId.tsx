  ...
  head: () => ({
    meta: [
      { title: "Suppliers · SupplierId — Grain Hero" },
      {
        name: "description",
        content:
          "Suppliers · SupplierId workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Suppliers · SupplierId — Grain Hero" },
      {
        property: "og:description",
        content: "Suppliers · SupplierId workspace in the Grain Hero platform.",
      },
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-xl font-semibold tabular-nums">{batches.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Total kg</div>
            <div className="text-xl font-semibold tabular-nums">{total_kg.toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Avg cost / kg</div>
            <div className="text-xl font-semibold tabular-nums">
              {avg_cost != null ? avg_cost.toFixed(2) : "—"}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-sm">{[s.phone, s.email].filter(Boolean).join(" · ") || "—"}</div>
          </CardContent>
        </Card>
      </div>
                ...
                batches.map((b) => (
                  <tr key={b.id} className="hover:bg-emerald-500/5">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums">
                      {Number(b.quantity_kg ?? 0).toLocaleString()} kg
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums">
