...
type S = {
  ...
  featured_commodities: string[]; min_app_build: number;
  kill_switch: boolean; kill_switch_message: string;
  allowed_attachment_types: string[]; max_message_length: number;
  moderation_banner: string;
};
const DEFAULTS: S = {
  hero_headline: "", hero_subheadline: "",
  featured_commodities: [], min_app_build: 1,
  kill_switch: false, kill_switch_message: "",
  allowed_attachment_types: ["image/jpeg","image/png","application/pdf"],
  max_message_length: 2000, moderation_banner: "",
};
...
function MarketplaceMobilePage() {
  ...
  useEffect(() => {
    if (data) {
      ...
      setS({
        ...
        allowed_attachment_types: (d.allowed_attachment_types as string[]) ?? DEFAULTS.allowed_attachment_types,
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { toast.success("Marketplace mobile settings saved"); qc.invalidateQueries({ queryKey: ["mkt-mobile"] }); },
  ...
  return (
    ...
    >
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>Storefront hero</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <F label="Headline"><Input value={s.hero_headline} onChange={(e) => patch({ hero_headline: e.target.value })} /></F>
            <F label="Subheadline"><Textarea rows={2} value={s.hero_subheadline} onChange={(e) => patch({ hero_subheadline: e.target.value })} /></F>
            <F label="Featured commodities (comma separated)">
              <Input value={s.featured_commodities.join(",")}
                onChange={(e) => patch({ featured_commodities: e.target.value.split(",").map((x) => x.trim()).filter(Boolean) })} />
            </F>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>Client &amp; availability</CardTitle></CardHeader>
          <CardContent className="grid gap-3">
            <F label="Minimum app build"><Input type="number" value={s.min_app_build} onChange={(e) => patch({ min_app_build: Number(e.target.value) })} /></F>
            <div className="flex items-center justify-between rounded border p-3">
              <div><div className="font-medium text-sm">Kill switch</div><div className="text-xs text-muted-foreground">Temporarily disable marketplace on mobile</div></div>
              <Switch checked={s.kill_switch} onCheckedChange={(v) => patch({ kill_switch: v })} />
            </div>
            <F label="Kill switch message"><Textarea rows={2} value={s.kill_switch_message} onChange={(e) => patch({ kill_switch_message: e.target.value })} /></F>
          </CardContent>
        </Card>
        <Card className="xl:col-span-2">
          <CardHeader><CardTitle>Messaging &amp; moderation</CardTitle></CardHeader>
          <CardContent className="grid gap-3 lg:grid-cols-2">
            <F label="Max message length"><Input type="number" value={s.max_message_length} onChange={(e) => patch({ max_message_length: Number(e.target.value) })} /></F>
            <F label="Allowed attachment MIME (comma separated)">
              <Input value={s.allowed_attachment_types.join(",")}
                onChange={(e) => patch({ allowed_attachment_types: e.target.value.split(",").map((x) => x.trim()).filter(Boolean) })} />
            </F>
            <F label="Moderation banner"><Textarea rows={2} value={s.moderation_banner} onChange={(e) => patch({ moderation_banner: e.target.value })} /></F>
          </CardContent>
        </Card>
      </div>
    </AdminPageShell>
