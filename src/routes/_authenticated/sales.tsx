...
} from "@/lib/buyer-orders.functions";
...
export const Route = createFileRoute("/_authenticated/sales")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sales workspace in the Grain Hero platform — private, sign-in required.",
      },
  ...
  const tiles = useMemo(
    () => [
      ...
      { key: "open", label: "Open orders", value: s?.openOrders ?? 0 },
      {
        key: "rev",
        label: "Revenue (30d)",
        value: `$${Number(s?.revenue30d ?? 0).toLocaleString()}`,
        hint: "paid invoices",
      },
      {
        key: "out",
        label: "Outstanding",
        value: `$${Number(s?.outstanding ?? 0).toLocaleString()}`,
                ...
                {rows.map((o: any) => (
                  <TableRow key={o.id} className="hover:bg-emerald-50/40">
                    ...
                    <TableCell className="max-w-[220px] truncate">
                      {o.grain_listings?.title ?? "—"}
                    </TableCell>
                    <TableCell className="text-right">
                      {Number(o.quantity_kg).toLocaleString()}
                    </TableCell>
                    <TableCell className="text-right">
...
function OrderDrawer({
  ...
  const nextQ = useQuery({
    queryKey: ["order-next", orderId],
    queryFn: () => nextFn({ data: { orderId } }),
  });
  ...
  const transMut = useMutation({
    mutationFn: (to: string) =>
      transitionFn({ data: { orderId, toState: to as any, note: note || undefined } }),
    onSuccess: () => {
  ...
  const invMut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(r.existed ? "Invoice already exists" : `Invoice ${r.invoiceNumber} generated`);
      refresh();
    },
    onError: (e: Error) => toast.error(e.message),
  });
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <div className="font-medium">{o.buyers?.company_name ?? o.buyers?.name}</div>
              </div>
              <div>
                <div className="text-muted-foreground">Listing</div>
                <div className="font-medium">{o.grain_listings?.title}</div>
              </div>
              <div>
                <div className="text-muted-foreground">Quantity</div>
                <div className="font-medium">{Number(o.quantity_kg).toLocaleString()} kg</div>
              </div>
              <div>
            ...
            {(nextQ.data?.next ?? []).length > 0 && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Advance</CardTitle>
                </CardHeader>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2">
                    <DollarSign className="w-4 h-4" /> Invoice {inv.invoice_number}
                  </CardTitle>
                </CardHeader>
                      ...
                      {payments.map((p: any) => (
                        <div key={p.id} className="flex justify-between text-muted-foreground">
                          ...
                          <span className="font-medium">
                            {p.currency ?? inv.currency} {Number(p.amount).toLocaleString()}
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-1 text-xs">
                ...
                {events.map((e: any) => (
                  <div
                    key={e.id}
                    className="flex justify-between text-muted-foreground border-b py-1 last:border-0"
                  >
                    <span>
                      {e.from_state ?? "—"} → <b>{e.to_state}</b>
                      {e.note ? ` · ${e.note}` : ""}
                    </span>
