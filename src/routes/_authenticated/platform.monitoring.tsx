  ...
  head: () => ({
    meta: [
      { title: "Platform · Monitoring — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Monitoring workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Monitoring — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Monitoring workspace in the Grain Hero platform.",
      },
...
function SparkCard({
  ...
  return (
    <div className="rounded-md border border-border bg-background p-3 flex flex-col gap-1.5">
      ...
      <span className="text-2xl font-medium tabular-nums" style={{ color }}>
        {value}
      </span>
      <div className="h-8">
        <ResponsiveContainer width="100%" height="100%">
...
function TenantRow({ t, rank }: { t: any; rank: number }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
      <span className="w-4 text-[9px] text-muted-foreground tabular-nums shrink-0 text-right">
        {rank}
      </span>
      ...
      <span className="w-7 text-[9px] tabular-nums text-right shrink-0" style={{ color: bar }}>
        {pct}%
      </span>
...
function PlatformMonitoringPage() {
  ...
  const incQ = useQuery({
    queryKey: ["platform-incidents-all"],
    queryFn: () => incFn({ data: { scope: "all" } }),
    staleTime: 30_000,
    refetchInterval: 30_000,
  });
  ...
  const updateMut = useMutation({
    mutationFn: (v: { id: string; status: (typeof MAINT_STATUSES)[number] }) =>
      updateFn({ data: v }),
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className="rounded-xl border border-border bg-background shadow-sm p-4 flex flex-col items-center gap-2 relative"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className="rounded-xl border border-border bg-background shadow-sm p-4 flex flex-col gap-2 relative"
              >
                <div className="absolute top-3 right-3 w-1.5 h-1.5 rounded-full bg-muted-foreground/20" />
          ...
          <div className="rounded-xl border border-border bg-background shadow-sm overflow-hidden">
            ...
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className="px-4 py-3 flex items-center gap-5 border-b border-border last:border-0"
              >
                <Sk className="h-[13px] w-28" />
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <DonutStat
              ...
              sub="unresolved"
              value={incTotals?.open ?? 0}
              total={incTotals?.total || 1}
              color={NEON.warning}
            />
          </div>
                ...
                <div className="flex items-center gap-3 text-[10px]">
                  <span className="flex items-center gap-1" style={{ color: NEON.success }}>
                    <span
                      ...
                      style={{ background: NEON.success }}
                    />{" "}
                    {totals?.online ?? 0} online
                  </span>
                  <span className="flex items-center gap-1" style={{ color: NEON.critical }}>
                    <span
                      className="w-1.5 h-1.5 rounded-full inline-block"
                      style={{ background: NEON.critical }}
                    />{" "}
                    {totals?.offline ?? 0} offline
                  </span>
                </div>
              }
                    ...
                    {maints.map((r) => (
                      ...
                      >
                        <td className="px-3 py-2">
                          ...
                          <div className="text-[11px] text-muted-foreground truncate max-w-[160px] sm:max-w-none">
                            {r.title}
                          </div>
                          {/* Mobile: show priority + date */}
                          <div className="sm:hidden text-[10px] text-muted-foreground mt-0.5 flex gap-2">
                            <span className="capitalize px-1 py-0.5 rounded border border-border">
                              {r.priority}
                            </span>
                        ...
                        <td className="px-3 py-2 hidden sm:table-cell">
                          <span className="capitalize text-[11px] px-1.5 py-0.5 rounded border border-border text-muted-foreground">
                            {r.priority}
                          </span>
                        </td>
                        ...
                        <td className="px-3 py-2 text-right">
                          <Select
                            ...
                            onValueChange={(v) =>
                              updateMut.mutate({
                                id: r.id,
                                status: v as (typeof MAINT_STATUSES)[number],
                              })
                            }
                          >
                            <SelectTrigger
                              className={`h-6 w-[100px] sm:w-[124px] text-[10px] border-0 font-medium ${maintTone(r.status)}`}
                            >
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
    ...
    <>
      ...
      {incByTenant.length === 0 ? (
        <div className="px-3 pt-3">
          <ChartEmpty label="No incidents" height={52} />
        </div>
      ) : (
                ...
                .map((t) => (
                  ...
                  >
                    ...
                    <td
                      ...
                      style={{ color: NEON.warning }}
                    >
                      {t.open}
                    </td>
                    <td className="px-2 py-2 text-right">
                      {t.critical > 0 ? (
                        <span className="font-medium tabular-nums" style={{ color: NEON.critical }}>
                          {t.critical}
                        </span>
                      ) : (
