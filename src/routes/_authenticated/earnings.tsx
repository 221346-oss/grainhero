...
export const Route = createFileRoute("/_authenticated/earnings")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Earnings workspace in the Grain Hero platform — private, sign-in required.",
      },
...
function EarningsPage() {
  ...
  const { data: balance, isLoading: l1 } = useQuery({
    queryKey: ["my-balance"],
    queryFn: () => bal({ data: {} }),
  });
  const { data: entries } = useQuery({
    queryKey: ["my-ledger"],
    queryFn: () => ledger({ data: { limit: 50 } }),
  });
  const { data: payoutList } = useQuery({
    queryKey: ["my-payouts"],
    ...
    <LocalizedContent>
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Your payable balance, payout history, and account details.
        </p>
      </div>
          ...
          <Card key={t.label}>
            <CardContent className="p-4 space-y-2">
              ...
              <div className="text-xl font-bold tabular-nums">
                {cur} {Number(t.value).toLocaleString()}
              </div>
            </CardContent>
          </Card>
        ))}
        ...
        <CardContent className="space-y-3">
          {acctData?.account ? (
            <div className="text-sm text-muted-foreground">
              Method: <span className="text-foreground font-medium">{acctData.account.method}</span>{" "}
              • {acctData.account.bank_name ?? "—"} • {acctData.account.currency}
            </div>
          ) : (
            <div className="text-sm text-muted-foreground">
              No payout account yet. Add one below to receive payments.
            </div>
          )}
          ...
          <Button size="sm" onClick={savePayoutAccount}>
            Save account
          </Button>
        </CardContent>
      </Card>
