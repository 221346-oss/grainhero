...
function OrderDetailPage() {
  ...
  const { data: techData } = useQuery({ queryKey: ["platform.technicians"], queryFn: () => fetchTechs() });
  ...
  const assign = useMutation({
    mutationFn: (v: { technicianId: string; scheduledFor?: string }) =>
      assignFn({ data: { orderId, technicianId: v.technicianId, scheduledFor: v.scheduledFor || null } }),
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">Technician</CardTitle></CardHeader>
          <CardContent className="text-sm space-y-1">
            {technician ? (
              <>
        ...
        <CardContent>
          {devices.length === 0 ? (
            ...
            <ul className="divide-y">
              {devices.map((d) => (
                <li key={d.id as string} className="py-2 flex items-center justify-between text-sm">
                  <span>{d.device_type ?? "device"} · {d.serial_number ?? <span className="italic text-muted-foreground">no serial</span>}</span>
                  {d.commissioned_at ? (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">Installations</CardTitle></CardHeader>
          <CardContent className="text-sm">
            {installs.length === 0 ? <div className="text-muted-foreground">None scheduled</div> :
              installs.map((i) => (
                <div key={i.id as string} className="flex items-center justify-between py-1.5 border-b last:border-0">
                  <div>
                    <div className="font-medium capitalize">{i.status?.replace("_"," ")}</div>
                    {i.scheduled_for && <div className="text-muted-foreground">{new Date(i.scheduled_for).toLocaleString()}</div>}
                    {i.blocker_note && <div className="text-rose-700">Blocked: {i.blocker_note}</div>}
                  </div>
                </div>
              ))}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="text-sm max-h-64 overflow-y-auto">
            {history.length === 0 ? <div className="text-muted-foreground">No transitions yet</div> :
              history.map((h) => (
                <div key={h.id as string} className="py-1.5 border-b last:border-0">
                  <div className="font-medium">{h.from_status} → {h.to_status}</div>
