...
export const Route = createFileRoute("/_authenticated/returns")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Returns workspace in the Grain Hero platform — private, sign-in required.",
      },
...
function ReturnsPage() {
  ...
  const mApprove = useMutation({
    mutationFn: () =>
      approve({ data: { returnId: openId!, resolutionKey, note: note || undefined } }),
    onSuccess: () => {
              ...
              {data?.returns?.map((r) => (
                <tr key={r.id as string} className="border-b hover:bg-emerald-50/30">
                  <td className="p-3 font-mono text-xs">
                    {(r.buyer_orders as { order_number?: string } | null)?.order_number ??
          ...
          {detailData?.return && (
            <div className="mt-4 space-y-4 text-sm">
              <div>
                ...
                {detailData.return.reason_label as string}
              </div>
              <div>
                <span className="text-muted-foreground">Notes: </span>
                {(detailData.return.notes as string) || "—"}
              </div>
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="capitalize">
                  {detailData.return.status as string}
                </Badge>
                {detailData.return.resolution && (
                  <Badge className="capitalize">{detailData.return.resolution as string}</Badge>
                )}
              </div>
              {Array.isArray(detailData.return.attachments) &&
                (detailData.return.attachments as unknown[]).length > 0 && (
                  <div className="text-xs text-muted-foreground">
                    {(detailData.return.attachments as Array<{ name: string }>).length}{" "}
                    attachment(s)
                  </div>
                )}
                  ...
                  {detailData.events?.map((e: Record<string, unknown>) => (
                    <li key={String(e.id)}>
                      ...
                      <span className="capitalize">
                        {e.from_state ? `${String(e.from_state)} → ` : ""}
                        {String(e.to_state)}
                      </span>
                ...
                {detailData.return.status === "requested" && (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        disabled={!resolutionKey || mApprove.isPending}
                      ...
                      <Button
                        size="sm"
                        variant="destructive"
                        disabled={mDeny.isPending}
                        onClick={() => mDeny.mutate()}
                      >
                        Deny
                      </Button>
                    </div>
                ...
                {detailData.return.status === "approved" && (
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      variant="secondary"
                      disabled={mReceived.isPending}
                      onClick={() => mReceived.mutate()}
                    >
                      Mark received
                    </Button>
                    <Button
