  ...
  head: () => ({
    meta: [
      { title: "Platform · Dashboard Builder — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Dashboard Builder workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Dashboard Builder — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Dashboard Builder workspace in the Grain Hero platform.",
      },
    ...
    mutationFn: () =>
      save({
        data: {
          ...
          role_scope: (role || undefined) as
            "super_admin" | "admin" | "manager" | "technician" | "buyer" | undefined,
          personal,
    ...
    >
              ...
              <SelectContent>
                {metrics.map((m) => (
                  <SelectItem key={m.key} value={m.key}>
                    {m.label} <span className="text-slate-400">({m.key})</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      <AdminDataCard title="Live preview">
        {widgets.length === 0 ? (
          <div className="p-8 text-center text-slate-400">
            No widgets yet. Add one above to see it here.
          </div>
        ) : (
      ...
      <Card>
        <CardContent className="p-4 text-xs text-slate-500">
          Every widget renders through <code>public.run_metric()</code>. Metrics visible here follow
          the role allow-list defined in <b>Metric Registry</b>. Nothing is hardcoded — edit or add
          metrics there and they appear immediately in this builder.
        </CardContent>
      </Card>
    </AdminPageShell>
  );
