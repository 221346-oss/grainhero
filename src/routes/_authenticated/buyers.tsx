...
const STATUS_CLASS: Record<Status, string> = {
  active: "bg-emerald-100 text-emerald-800 border-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-300 dark:border-emerald-500/30",
  paused: "bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-500/15 dark:text-amber-300 dark:border-amber-500/30",
...
function StatCard({ label, value, icon, tone, suffix }: { label: string; value: number; icon: React.ReactNode; tone: "indigo"|"emerald"|"amber"|"blue"; suffix?: string }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-3">
      ...
      <div className={`text-2xl font-bold mt-1 ${valueTone[tone]}`}>{value}{suffix ?? ""}</div>
    </div>
  );
}
