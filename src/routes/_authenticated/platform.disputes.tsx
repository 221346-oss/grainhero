  ...
  head: () => ({
    meta: [
      { title: "Platform · Disputes — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Disputes workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Disputes — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Disputes workspace in the Grain Hero platform.",
      },
...
function PlatformDisputesPage() {
  ...
  const [status, setStatus] = useState<"open" | "under_review" | "resolved" | "rejected" | "all">(
    "open",
  );
  const { data } = useQuery({
    ...
    mutationFn: async () => {
      ...
      if (selectedRez && selectedRez.refund !== "none" && data) {
        const amt = refundAmount
          ? Number(refundAmount)
          : Number((data.dispute.buyer_orders as { subtotal?: number } | null)?.subtotal ?? 0);
        if (amt > 0) {
          await refundFn({
            data: {
  ...
  return (
    <Sheet open onOpenChange={(v) => !v && onClose()}>
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Dispute #{id.slice(0, 8)}</SheetTitle>
        </SheetHeader>
        {!d ? (
          ...
          <div className="space-y-3 text-sm mt-4">
            ...
            <div className="rounded border p-2 bg-muted/40 whitespace-pre-wrap">
              {d.description}
            </div>
