...
const ENTRY_TYPES = ["", "payment_in","refund_out","platform_fee","logistics_cost","tax","payout_out","adjustment"];
                ...
                <tbody>
                  {(data?.rows ?? []).map((r: any) => (
                    <tr key={r.id} className="border-t hover:bg-muted/20">
                      ...
                      <td className="p-3">
                        <span className={r.direction === "credit" ? "text-emerald-600" : "text-rose-600"}>
                          {r.direction === "credit" ? "+" : "−"}
                        </span>
                      </td>
                      <td className="p-3 text-right font-mono">{r.currency} {Number(r.amount).toLocaleString()}</td>
