  ...
  head: () => ({
    meta: [
      { title: "Platform · Subscriptions — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Subscriptions workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Subscriptions — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Subscriptions workspace in the Grain Hero platform.",
      },
...
function PlatformSubscriptionsPage() {
  const fetchRoleFn = useServerFn(getMyRole);
  const roleQ = useQuery({ queryKey: ["my-role"], queryFn: () => fetchRoleFn() });
    ...
    return (
      <AdminPageShell title="Subscriptions" subtitle="Access restricted">
        <div className="p-8 text-center text-sm text-muted-foreground">
          Subscriptions workspace is available to super admins only.
        </div>
      </AdminPageShell>
    );
  }
  ...
  if (isLoading)
    return (
      <AdminPageShell title="Subscriptions" subtitle="Platform subscriptions">
              ...
              {allSubs.map((s: any) => {
                ...
                return (
                  <tr key={s.id} className="border-b hover:bg-slate-50 last:border-b-0">
                    ...
                    <td className={`px-4 py-3 text-[10px] font-medium ${expiryColor}`}>
                      {expiryText}
                    </td>
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              runAdmin(
                                () =>
                                  adminCancelFn({
                                    data: { subscriptionId: s.id, immediate: false },
                                  }),
                          ...
                          <DropdownMenuItem
                            ...
                            onClick={() =>
                              runAdmin(
                                () =>
                                  adminCancelFn({
                                    data: { subscriptionId: s.id, immediate: true },
                                  }),
