...
type Row = { id?: string; key: string; native_route: string; web_fallback: string; description?: string; active: boolean };
...
function MobileDeepLinksPage() {
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["mobile-deep-links"] }); },
  });
  ...
  return (
    ...
    >
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>Add / update route</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <Field label="Key (e.g. alert.detail)">
              <Input value={draft.key} onChange={(e) => setDraft({ ...draft, key: e.target.value })} />
            </Field>
            <Field label="Native route (:params allowed)">
              <Input value={draft.native_route} onChange={(e) => setDraft({ ...draft, native_route: e.target.value })} />
            </Field>
            <Field label="Web fallback">
              <Input value={draft.web_fallback} onChange={(e) => setDraft({ ...draft, web_fallback: e.target.value })} />
            </Field>
            ...
            <Button onClick={() => saveMut.mutate(draft)} disabled={saveMut.isPending || !draft.key}>Save</Button>
          </CardContent>
        </Card>
              ...
              <div key={r.id ?? r.key} className="rounded-md border p-3 text-sm">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    {r.id && <Button size="sm" variant="ghost" onClick={() => delMut.mutate(r.id!)}>Delete</Button>}
                  </div>
                </div>
                <div className="mt-1 text-xs text-muted-foreground">
                  <div>native: <span className="font-mono">{r.native_route}</span></div>
                  <div>web: <span className="font-mono">{r.web_fallback}</span></div>
