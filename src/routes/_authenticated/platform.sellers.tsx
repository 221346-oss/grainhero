  ...
  head: () => ({
    meta: [
      { title: "Platform · Sellers — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Sellers workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Sellers — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Sellers workspace in the Grain Hero platform.",
      },
                  ...
                  <tr key={s.admin_id} className="border-b last:border-0 hover:bg-emerald-50/40">
                    <td className="p-3">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {s.profile?.city ?? ""} {s.profile?.country ?? ""}
                      </div>
                    </td>
                    <td>
                      <span
                    ...
                    <td>
                      {Number(s.avg_rating || 0).toFixed(2)} ({s.review_count_total})
                    </td>
                    <td>{s.delivered_count}</td>
                    <td>{Math.round((Number(s.on_time_rate) || 0) * 100)}%</td>
                    <td>
                      {s.dispute_count} ({Math.round((Number(s.dispute_rate) || 0) * 100)}%)
                    </td>
                    <td>
                ...
                {(data?.sellers ?? []).length === 0 && (
                  <tr>
                    <td colSpan={7} className="p-6 text-center text-muted-foreground">
                      No sellers with delivered orders yet.
                    </td>
                  </tr>
                )}
              </tbody>
