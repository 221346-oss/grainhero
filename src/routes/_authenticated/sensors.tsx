...
import {
  ...
  listLatestSensorReadings, listDeviceReadings,
  listWarehouses, listSilos,
...
const SENSOR_TYPES = ["temperature","humidity","co2","voc","moisture","pressure","light","ph"] as const;
type SensorType = typeof SENSOR_TYPES[number];

type Device = {
  id: string; device_id: string; device_name: string;
  mac_address: string | null; model: string | null; manufacturer: string | null;
  firmware_version: string | null; device_type: string | null;
  ...
  notes: string | null;
  warehouse_id: string; silo_id: string | null;
...
type Form = {
  ...
  device_name: string;
  mac_address: string;
  model: string; manufacturer: string; firmware_version: string;
  device_type: string; category: string;
  sensor_types: SensorType[];
  warehouse_id: string; silo_id: string;
  status: "active"|"offline"|"error"|"maintenance";
...
const emptyForm: Form = {
  device_name: "", mac_address: "", model: "", manufacturer: "",
  firmware_version: "", device_type: "environmental", category: "environmental",
  sensor_types: ["temperature","humidity"],
  warehouse_id: "", silo_id: "",
  status: "active", power_source: "",
...
function SensorsPage() {
  ...
  const { data: warehousesData } = useQuery({ queryKey: ["warehouses"], queryFn: () => listWhFn() as Promise<Warehouse[]> });
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((d) => {
      ...
      return d.device_name?.toLowerCase().includes(t) ||
  ...
  const saveMut = useMutation({
    mutationFn: (f: Form) => upsertFn({ data: {
      ...
      silo_id: f.silo_id,
      status: f.status,
      power_source: (f.power_source || null) as ("solar"|"battery"|"direct"|"hybrid"|null),
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-5">
        ...
        <div className="grid grid-cols-2 sm:flex gap-2">
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            <SelectTrigger className="w-full sm:w-36"><SelectValue placeholder="Sensor type" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All types</SelectItem>
              {SENSOR_TYPES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
            </SelectContent>
          </Select>
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {rows.map((d) => (
            <SensorCard
              key={d.id} device={d} reading={readingByDevice.get(d.id) ?? null}
        ...
        <DialogContent className="max-w-lg max-h-[92vh] overflow-y-auto">
          ...
          <form id="sensor-form" className="grid gap-3 py-2" onSubmit={(e) => { e.preventDefault(); saveMut.mutate(form); }}>
            <div>
              ...
              <Input required value={form.device_name} onChange={(e) => setForm({ ...form, device_name: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Warehouse *</Label>
                <Select value={form.warehouse_id} onValueChange={(v) => setForm({ ...form, warehouse_id: v, silo_id: "" })}>
              ...
              <div>
                ...
                <Select value={form.silo_id} onValueChange={(v) => setForm({ ...form, silo_id: v })}>
                  <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent>
                    {filteredSilos.map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {SENSOR_TYPES.map(t => {
                  ...
                  return (
                    <button
                      type="button" key={t}
                      onClick={() => setForm({ ...form, sensor_types: on ? form.sensor_types.filter(x => x !== t) : [...form.sensor_types, t] })}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input value={form.mac_address} onChange={(e) => setForm({ ...form, mac_address: e.target.value })} placeholder="AA:BB:CC:…" />
              </div>
              <div>
              ...
              <div>
                ...
                <Select value={form.power_source || "none"} onValueChange={(v) => setForm({ ...form, power_source: v === "none" ? "" : v as Form["power_source"] })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Input type="number" min={1} value={form.data_transmission_interval} onChange={(e) => setForm({ ...form, data_transmission_interval: e.target.value })} />
              </div>
              <div>
                <Label>Calibration (days)</Label>
                <Input type="number" min={1} value={form.calibration_interval_days} onChange={(e) => setForm({ ...form, calibration_interval_days: e.target.value })} />
              </div>
              ...
              <label className="col-span-2 flex items-center gap-2 text-sm text-slate-700">
                <Checkbox checked={form.is_enabled} onCheckedChange={(v) => setForm({ ...form, is_enabled: !!v })} /> Device enabled
              </label>
              <div className="col-span-2">
                <Label>Notes</Label>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Historical readings remain but the device link will be broken.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
    ...
    <Card className="border-slate-200/70 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      <CardContent className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          ...
          <MiniReading icon={AlertTriangle} value={reading?.voc_value} unit="" tone={reading?.anomaly_detected ? "warn" : undefined} />
        </div>
...
function DeviceDetail({ device, reading, historyFn, onEdit }: {
  device: Device; reading: Reading | null;
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>{device.device_id}{device.model ? ` · ${device.model}` : ""}</DialogDescription>
      </DialogHeader>
      <div className="space-y-3 text-sm py-2">
        <Row label="Status"><StatusBadge value={device.status} /></Row>
