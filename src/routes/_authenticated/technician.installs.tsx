      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Orders assigned to you for on-site installation.
          </p>
        </div>
        {/* Export buttons */}
        ...
        <div className="grid gap-3">
          {installs.map((i) => {
            ...
            return (
              <Link
                key={i.id as string}
                to="/technician/installs/$installId"
                params={{ installId: i.id as string }}
                className="block"
              >
                <Card className="hover:border-emerald-400 transition-colors">
                  <CardHeader className="pb-2 flex flex-row items-center justify-between">
                    ...
                    <Badge className={STATUS_COLOR[i.status as string] ?? "bg-slate-200"}>
