...
} from "@/lib/mobile-commerce-settings.functions";
...
const DEFAULTS: S = {
  ...
  stripe_publishable_key_override: null,
  cod_max_cents: 0,
  ...
  head: () => ({
    meta: [
      { title: "Platform · Commerce Mobile — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Commerce Mobile workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Commerce Mobile — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Commerce Mobile workspace in the Grain Hero platform.",
      },
...
function CommerceMobilePage() {
    ...
    if (data) {
      ...
      setS({
        ...
        allowed_payment_methods:
          (d.allowed_payment_methods as string[]) ?? DEFAULTS.allowed_payment_methods,
        ...
        stripe_publishable_key_override:
          (d.stripe_publishable_key_override as string | null) ?? null,
  ...
  const mut = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["mobile-commerce-settings"] });
    },
  ...
  return (
    ...
    >
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded border p-3">
              <div>
                <div className="font-medium text-sm">Checkout enabled</div>
                <div className="text-xs text-muted-foreground">
                  Turn mobile buyer checkout on or off
                </div>
              </div>
              <Switch
                checked={s.checkout_enabled}
                onCheckedChange={(v) => patch({ checkout_enabled: v })}
              />
            </div>
            <F label="Allowed payment methods (comma separated)">
              <Input
                value={s.allowed_payment_methods.join(",")}
                onChange={(e) =>
                  patch({
            ...
            <F label="Default currency (ISO)">
              <Input
                value={s.currency_default}
                onChange={(e) => patch({ currency_default: e.target.value.toLowerCase() })}
              />
            </F>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Limits &amp; fees</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-3">
            <F label="Min order (cents)">
              <Input
                type="number"
                value={s.min_order_cents}
                onChange={(e) => patch({ min_order_cents: Number(e.target.value) })}
              />
            </F>
            <F label="Max order (cents)">
              <Input
                type="number"
                value={s.max_order_cents}
                onChange={(e) => patch({ max_order_cents: Number(e.target.value) })}
              />
            </F>
            <F label="Platform fee (bps)">
              <Input
                type="number"
                value={s.platform_fee_bps}
                onChange={(e) => patch({ platform_fee_bps: Number(e.target.value) })}
              />
            </F>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Checkout behaviour</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-2">
            <F label="Cash-on-delivery max (cents, 0 = disabled)">
              <Input
                type="number"
                value={s.cod_max_cents}
                onChange={(e) => patch({ cod_max_cents: Number(e.target.value) })}
              />
            </F>
            <F label="Quote TTL (seconds)">
              <Input
                type="number"
                value={s.quote_ttl_seconds}
                onChange={(e) => patch({ quote_ttl_seconds: Number(e.target.value) })}
              />
            </F>
          </CardContent>
        </Card>
        <Card className="xl:col-span-2">
          <CardHeader>
            <CardTitle>Legal &amp; overrides</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-3">
            <F label="Terms URL">
              <Input
                value={s.terms_url ?? ""}
                onChange={(e) => patch({ terms_url: e.target.value || null })}
              />
            </F>
            <F label="Refund policy URL">
              <Input
                value={s.refund_policy_url ?? ""}
                onChange={(e) => patch({ refund_policy_url: e.target.value || null })}
              />
            </F>
            <F label="Stripe publishable key (optional override)">
              <Input
                value={s.stripe_publishable_key_override ?? ""}
                onChange={(e) => patch({ stripe_publishable_key_override: e.target.value || null })}
              />
            </F>
          </CardContent>
        </Card>
