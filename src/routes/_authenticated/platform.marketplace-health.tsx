  ...
  head: () => ({
    meta: [
      { title: "Platform · Marketplace Health — Grain Hero" },
      {
        name: "description",
        content:
          "Platform · Marketplace Health workspace in the Grain Hero platform — private, sign-in required.",
      },
      { property: "og:title", content: "Platform · Marketplace Health — Grain Hero" },
      {
        property: "og:description",
        content: "Platform · Marketplace Health workspace in the Grain Hero platform.",
      },
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <SellerCard
              ...
              scoreLabel="Score"
              scoreKey="score"
              verifiedMin={data.verifiedThreshold}
            />
            <SellerCard
              title="Highest dispute rate"
              sellers={data.worstByDisputes}
              scoreLabel="Disputes"
              scoreKey="disputeRate"
              verifiedMin={data.verifiedThreshold}
              pct
            />
          </div>
  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm">
        {rows.length === 0 ? (
          <div className="text-muted-foreground">None in window.</div>
        ) : (
...
function SellerCard({
  ...
  title: string;
  sellers: Array<{ adminId: string; score: number; disputeRate: number; avgRating: number }>;
  scoreLabel: string;
  scoreKey: "score" | "disputeRate";
  verifiedMin: number;
  pct?: boolean;
}) {
  return (
            ...
            {sellers.map((s) => {
              ...
              return (
                <li key={s.adminId} className="flex justify-between items-center">
                  ...
                  <span
                    className={
                      s.score >= verifiedMin ? "text-emerald-700 font-semibold" : "text-foreground"
                    }
                  >
                    {scoreLabel}: {label}
                  </span>
