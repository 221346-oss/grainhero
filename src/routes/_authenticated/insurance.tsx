...
export const Route = createFileRoute("/_authenticated/insurance")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Insurance workspace in the Grain Hero platform — private, sign-in required.",
      },
...
function InsurancePage() {
  ...
  const { data: policies = [], isLoading: policiesLoading } = useQuery({
    ...
    queryFn: () => listPoliciesFn() as Promise<InsurancePolicyRow[]>,
  });
  const { data: claims = [], isLoading: claimsLoading } = useQuery({
    queryKey: ["insurance-claims"],
    queryFn: () => listClaimsFn() as Promise<InsuranceClaimRow[]>,
  });
    ...
    >
          ...
          <PlatformOverviewTable
            ...
            columns={[
              ...
              },
              {
                ...
                label: "Coverage",
                align: "right",
                render: (r) => `PKR ${r.coverage.toLocaleString()}`,
              },
              {
                key: "premium",
                label: "Premium",
                align: "right",
                render: (r) => `PKR ${r.premium.toLocaleString()}`,
              },
              {
                key: "openClaims",
                label: "Open claims",
                align: "right",
                render: (r) => (
                  <span className={r.openClaims > 0 ? "text-amber-700 font-medium" : ""}>
                    {r.openClaims}
                  </span>
                ),
              },
      ...
      <ResponsiveTabs value={activeTab} onValueChange={setActiveTab}>
        <ResponsiveTabsList className="mb-4">
          <ResponsiveTabsTrigger value="policies">
              ...
              {policiesLoading ? (
                ...
                <div className="divide-y divide-border">
                  {policies.map((p) => (
                    <div
                      key={p.id}
                      className="flex flex-wrap items-center justify-between gap-4 p-4 hover:bg-muted/30 transition-colors bg-white"
                    >
                      <div className="flex-1 min-w-[200px]">
                        <div className="font-bold text-slate-900 text-base">{p.policy_number}</div>
                        <div className="text-xs text-slate-500 font-medium mt-0.5">
                          {p.provider_name} · <span className="capitalize">{p.coverage_type}</span>
                        </div>
                      </div>
                      <div className="text-sm font-semibold text-slate-900">
                        <span className="text-xs font-normal text-slate-500 block">Coverage</span>
                        PKR {(p.coverage_amount ?? 0).toLocaleString()}
                      </div>
                      <div className="text-sm font-semibold text-slate-900">
                        <span className="text-xs font-normal text-slate-500 block">Premium</span>PKR{" "}
                    ...
                    >
                      <div className="flex-1 min-w-[200px]">
                        ...
                        <div className="text-xs text-slate-500 font-medium mt-0.5 truncate">
                          {c.claim_type} · {c.description ?? "—"}
                        </div>
                      </div>
                      <div className="text-sm font-semibold text-slate-900">
                        <span className="text-xs font-normal text-slate-500 block">Claimed</span>PKR{" "}
                        {(c.amount_claimed ?? 0).toLocaleString()}
                      </div>
                      <div className="text-sm font-semibold text-slate-900">
                        <span className="text-xs font-normal text-slate-500 block">Approved</span>
                        PKR {(c.amount_approved ?? 0).toLocaleString()}
                      </div>
                      <Badge
                      ...
                      <div className="flex items-center gap-1">
                        <Button size="sm" variant="ghost" onClick={() => openEditClaim(c)}>
                          <Edit2 className="h-4 w-4" />
                        </Button>
                        <Button
                          size="sm"
      ...
      <Dialog open={policyOpen} onOpenChange={setPolicyOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{policyForm.id ? "Edit policy" : "New policy"}</DialogTitle>
          </DialogHeader>
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setPolicyForm({ ...policyForm, coverage_type: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
      ...
      <Dialog open={claimOpen} onOpenChange={setClaimOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{claimForm.id ? "Edit claim" : "New claim"}</DialogTitle>
          </DialogHeader>
          <div className="grid md:grid-cols-2 gap-3">
            <div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setClaimForm({ ...claimForm, amount_approved: e.target.value })}
              />
            </div>
            <div>
