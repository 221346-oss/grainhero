...
export const Route = createFileRoute("/marketplace/seller/$adminId")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: "Public seller storefront with reputation, reviews, and active listings.",
      },
    ],
  }),
});
            ...
            >
              <div className="p-4">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {l.available_kg} kg · {brand.currency} {Number(l.price_per_kg).toFixed(2)}/kg
                </div>
              </div>
            </a>
          ))}
          ...
          {reviews.map((r) => (
            <Card key={r.id}>
              <CardContent className="p-4 space-y-1">
                ...
                <div className="text-xs text-muted-foreground">
                  {r.helpful_count ?? 0} people found this helpful
                </div>
              </CardContent>
            </Card>
          ))}
