    ...
    handlers: {
      POST: async ({ request, params }) => {
        ...
        const { data: carrier } = await sb
          .from("carriers")
          .select("id, webhook_secret, event_map, active")
          .eq("code", code)
          .maybeSingle();
        ...
        const trackingNumber = payload.tracking_number
          ? String(payload.tracking_number).slice(0, 120)
          : null;
        ...
        if (assignmentId) {
          ...
          if (canonical === "delivered") {
            ...
            patch.actual_delivery_at = occurredAt;
          }
          if (canonical === "exception" || canonical === "failed_delivery")
