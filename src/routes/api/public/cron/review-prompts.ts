...
export const Route = createFileRoute("/api/public/cron/review-prompts")({
  server: { handlers: {
    POST: async ({ request }) => {
      ...
      if (!settings.reviews.enabled) return Response.json({ ok: true, skipped: "reviews disabled" });
      const delayMs = (settings.reviews.promptDelayHours ?? 24) * 3_600_000;
      const cutoff = new Date(Date.now() - delayMs).toISOString();
      const { data: orders } = await sb.from("buyer_orders")
      ...
      for (const o of (orders ?? []) as Array<{ id: string; admin_id: string }>) {
        ...
        const { data: existing } = await sb.from("buyer_reviews")
          .select("id").eq("order_id", o.id).eq("direction", "buyer_to_seller").maybeSingle();
        if (existing) {
          await sb.from("buyer_orders").update({ review_prompt_sent_at: new Date().toISOString() } as never).eq("id", o.id);
          continue;
        }
        try {
