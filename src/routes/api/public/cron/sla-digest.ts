    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data: recent } = await sb
          .from("platform_email_digest_log")
          .select("id")
          .eq("digest_key", "sla_daily")
        ...
        const slaCutoff = new Date(
          now.getTime() - settings.dispatch.slaHours.inTransit * 3_600_000,
        ).toISOString();
        ...
        const [{ data: overdue }, { data: recentShip }] = await Promise.all([
          sb
            ...
            .select(
              "id, order_id, status, courier_label, dispatched_at, expected_delivery_at, buyer_orders(order_number)",
            )
        ...
        for (const to of recipients) {
          try {
            await sendEmailViaResend({
              to,
              subject: `[GrainHero] SLA digest — ${overdueList.length} overdue, ${rate}% delivered`,
              html,
              from: settings.fromEmail,
            });
            sent++;
          } catch (e) {
            console.warn("[sla-digest] email failed:", (e as Error).message);
          }
        }
