    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        for (const op of body.ops) {
          ...
          if (!handler) {
            results.push({ endpoint: op.endpoint, status: "error", error: "unknown_endpoint" });
            continue;
          }
          const endpointKey = `replay/${op.endpoint}`;
          const cached = await getIdempotent(
            ctx.supabase,
            ctx.userId,
            op.idempotency_key,
            endpointKey,
          ...
          if (cached) {
            results.push({ endpoint: op.endpoint, status: "cached", response: cached });
            continue;
          }
          try {
