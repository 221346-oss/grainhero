      ...
      POST: async ({ request }) => {
        ...
        return withSyncLogging(
          ...
          async () => {
            ...
            try { parsed = batchSchema.parse(await request.json()); }
            catch (e) { return { response: Response.json({ error: (e as Error).message }, { status: 400 }), rowCount: 0 }; }

            const results: Array<{ client_id: string; status: "ok" | "error" | "deduped"; result?: unknown; error?: string }> = [];
            let failureCount = 0;

            for (const m of parsed.mutations) {
              const handler = ACTIONS[m.kind];
              if (!handler) {
                results.push({ client_id: m.client_id, status: "error", error: `unknown_kind:${m.kind}` });
                failureCount++;
                continue;
              }
              const prior = await getIdempotent(ctx.supabase, ctx.userId, m.client_id, `field-mutations:${m.kind}`);
              if (prior !== null) {
                results.push({ client_id: m.client_id, status: "deduped", result: prior });
                continue;
              }
              try {
                const result = await handler({ supabase: ctx.supabase, userId: ctx.userId }, m.payload);
                await saveIdempotent(ctx.supabase, ctx.userId, m.client_id, `field-mutations:${m.kind}`, result);
                results.push({ client_id: m.client_id, status: "ok", result });
              } catch (err) {
                failureCount++;
                results.push({ client_id: m.client_id, status: "error", error: (err as Error).message?.slice(0, 500) });
              }
            }

            // Invalidate cached bundle so next fetch is fresh
            await ctx.supabase.from("mobile_field_bundles")
            ...
            return {
              response: Response.json({ data: { results }, meta: { server_time: new Date().toISOString(), version: "v1" } }),
