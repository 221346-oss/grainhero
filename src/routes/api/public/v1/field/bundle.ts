  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        return withSyncLogging({ endpoint: "field-bundle", actorUserId: ctx.userId }, async () => {
          const inm = request.headers.get("if-none-match");

          const { data: fsRow } = await ctx.supabase
          ...
          // Check cached bundle
          const { data: cachedRaw } = await ctx.supabase
          ...
          if (cached && new Date(cached.expires_at).getTime() > Date.now()) {
            if (inm && inm === cached.etag) {
              return {
                response: new Response(null, { status: 304, headers: { ETag: cached.etag } }),
                rowCount: 0,
              };
            }
            return {
          ...
          if (tasks.error)
            return {
              response: Response.json({ error: tasks.error.message }, { status: 500 }),
              rowCount: 0,
            };
          if (incidents.error)
            return {
              response: Response.json({ error: incidents.error.message }, { status: 500 }),
              rowCount: 0,
            };
