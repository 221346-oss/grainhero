  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        const { data, error } = await supabase.from("mobile_commerce_settings" as never).select("*").limit(1).maybeSingle();
        if (error) return Response.json({ error: error.message }, { status: 500 });
        const row = data as unknown as {
          checkout_enabled: boolean; allowed_payment_methods: string[];
          min_order_cents: number; max_order_cents: number; currency_default: string;
          terms_url: string | null; refund_policy_url: string | null;
