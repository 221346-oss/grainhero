    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        return withSyncLogging({ endpoint: "commerce-cart", actorUserId: ctx.userId }, async () => {
          ...
          return {
            response: Response.json({ data, meta: { version: "v1" } }),
            rowCount: data ? 1 : 0,
          };
        });
      },
      PUT: async ({ request }) => {
        ...
        return withSyncLogging({ endpoint: "commerce-cart", actorUserId: ctx.userId }, async () => {
          ...
          try {
            body = cartSchema.parse(await request.json());
          } catch (e) {
            return {
          ...
          const subtotalCents = body.items.reduce(
            (s, it) => s + Math.round(it.quantity_kg * it.unit_price_cents),
            0,
          );

          // Enforce commerce settings bounds
          const { data: cfgRaw } = await ctx.supabase
            .from("mobile_commerce_settings" as never)
            .select("min_order_cents,max_order_cents,checkout_enabled")
            .limit(1)
            .maybeSingle();
          const cfg = cfgRaw as {
          ...
          if (cfg && !cfg.checkout_enabled) {
            return {
              response: Response.json({ error: "checkout_disabled" }, { status: 403 }),
              rowCount: 0,
            };
          }
          ...
          const { data, error } = await ctx.supabase
            .from("buyer_carts")
            .upsert(payload as never, { onConflict: "buyer_id" })
            .select("*")
            .maybeSingle();
          if (error)
            return {
