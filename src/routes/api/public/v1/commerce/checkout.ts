    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const requestHash = createHash("sha256")
          .update(JSON.stringify({ u: ctx.userId, b: body }))
          .digest("hex");
        ...
        // Load cart
        const { data: cartRow, error: cartErr } = await ctx.supabase
          .from("buyer_carts")
        ...
        if (!cfg.checkout_enabled)
          return Response.json({ error: "checkout_disabled" }, { status: 403 });
        ...
        let quote;
        try {
          quote = await computeQuote(ctx.supabase, items, body.address_id, ctx.userId);
        } catch (e) {
          return Response.json({ error: (e as Error).message }, { status: 400 });
        }

        if (quote.total_cents < cfg.min_order_cents)
          return Response.json({ error: "below_min" }, { status: 400 });
        if (
          body.payment_method === "cod" &&
          cfg.cod_max_cents > 0 &&
          quote.total_cents > cfg.cod_max_cents
        ) {
          return Response.json({ error: "cod_limit_exceeded" }, { status: 400 });
        }

        // Load shipping address snapshot
        const { data: addr } = await ctx.supabase
        ...
        if (!buyerId || !buyerAccountId) {
          ...
          const { data: buyerRow, error: bErr } = await supabaseAdmin
            .from("buyers")
          ...
          if (bErr || !buyerRow)
            return Response.json(
              { error: `buyer_create_failed:${bErr?.message ?? "unknown"}` },
              { status: 500 },
            );
          buyerId = (buyerRow as { id: string }).id;
          const { data: acctIns, error: aErr } = await supabaseAdmin
            .from("buyer_accounts")
            .insert({
              user_id: ctx.userId,
              buyer_id: buyerId,
          ...
          if (aErr || !acctIns)
            return Response.json(
              { error: `buyer_account_create_failed:${aErr?.message ?? "unknown"}` },
              { status: 500 },
            );
        ...
        for (const line of quote.lines) {
          ...
          const { data: insertedRaw, error: insErr } = await supabaseAdmin
            .from("buyer_orders")
            .insert({
          ...
          if (insErr || !insertedRaw) {
            ...
            return Response.json(
              { error: `order_insert_failed:${insErr?.message ?? "unknown"}` },
              { status: 500 },
            );
          }
        ...
        await supabaseAdmin.from("mobile_idempotency_keys").insert({
          ...
          response: responseBody as never,
        } as never);
