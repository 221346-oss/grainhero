...
async function buyerIdFor(ctx: Awaited<ReturnType<typeof authenticateMobile>>) {
  if (ctx instanceof Response) return null;
  const { data } = await ctx.supabase.from("buyer_accounts").select("id").eq("user_id", ctx.userId).maybeSingle();
    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        return withSyncLogging({ endpoint: "commerce-addresses", actorUserId: ctx.userId }, async () => {
          const buyerId = await buyerIdFor(ctx);
          if (!buyerId) return { response: Response.json({ data: [], meta: { version: "v1" } }), rowCount: 0 };
