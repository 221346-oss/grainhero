  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const quote = await computeQuote(ctx.supabase, body.items, body.address_id ?? null, ctx.userId);
          return Response.json({ data: quote, meta: { version: "v1" } });
        } catch (e) {
          const msg = (e as Error).message;
          const status = msg === "checkout_disabled" ? 403
            : msg === "address_forbidden" ? 403
            : msg === "address_not_found" ? 404
            : msg === "cart_empty" || msg === "no_valid_lines" ? 400
            : 500;
