...
export const recordDispatchedWeight = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: order } = await sb.from("buyer_orders")
      .select("admin_id, order_number").eq("id", data.orderId).maybeSingle();
    ...
    const { data: existing } = await sb.from("buyer_order_weight_reconciliation")
      .select("id").eq("order_id", data.orderId).maybeSingle();
    const payload = {
    ...
    if (existing) {
      await sb.from("buyer_order_weight_reconciliation")
        .update(payload as never).eq("id", (existing as Row).id);
      return { id: (existing as Row).id as string };
    }
    const { data: ins, error } = await sb.from("buyer_order_weight_reconciliation")
...
export const recordReceivedWeight = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rec } = await sb.from("buyer_order_weight_reconciliation")
    ...
    if (existing) {
      ...
      const { data: order } = await sb.from("buyer_orders")
        .select("admin_id").eq("id", data.orderId).maybeSingle();
      const o = order as Row | null;
      if (!o) throw new Error("Order not found");
      await sb.from("buyer_order_weight_reconciliation").insert({
        ...payload, admin_id: o.admin_id,
      } as never);
    }
    ...
    if (flagged) {
      ...
      if (o) {
        void emitNotification(sb, {
          recipientId: o.admin_id as string, tenantAdminId: o.admin_id as string,
