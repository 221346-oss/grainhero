...
export const listMyShares = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await sb.from("dashboard_shares").select("*").order("created_at", { ascending: false }).limit(200);
...
export const createShare = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await sb.from("dashboard_shares").insert({
    ...
    await sb.rpc("record_governance_audit", {
      _action: "share.create", _target_type: "share", _target_key: row.id, _before: null, _after: row,
    });
    return { share: row };
  });
...
export const revokeShare = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = context.supabase as any;
    const { error } = await sb.from("dashboard_shares").update({ revoked_at: new Date().toISOString() }).eq("id", data.id);
    if (error) throw error;
    await sb.rpc("record_governance_audit", {
      _action: "share.revoke", _target_type: "share", _target_key: data.id, _before: null, _after: null,
    });
    return { ok: true };
  });
...
export const resolveShare = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: meta, error } = await admin.rpc("resolve_dashboard_share", { _token: data.token });
    if (error) throw error;
    ...
    for (const m of (metrics ?? []) as Array<{ key: string; sql_template: string }>) {
      try {
        ...
        const { data: raw } = await admin.rpc("run_metric_as_admin", { _key: m.key }).catch(() => ({ data: null }));
