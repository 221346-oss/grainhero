...
export const listBatchCertificates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows, error } = await sb.from("batch_quality_certificates")
      .select("*").eq("batch_id", data.batchId)
    ...
    for (const r of (rows ?? []) as Row[]) {
      ...
      if (r.file_path) {
        const { data: s } = await sb.storage.from("quality-certificates")
...
export const addBatchCertificate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = context.supabase as any;
    const { data: batch } = await sb.from("grain_batches")
      .select("id, admin_id, batch_id").eq("id", data.batchId).maybeSingle();
    ...
    if (data.metrics && settings.quality.metrics.length) {
      ...
      for (const m of settings.quality.metrics) {
        ...
        if (m.min != null && v < m.min) { ok = false; break; }
        if (m.max != null && v > m.max) { ok = false; break; }
      }
      status = ok ? "passed" : "failed";
    }

    const expires = settings.quality.certificateValidityDays > 0
      ? new Date(Date.now() + settings.quality.certificateValidityDays * 86400_000).toISOString()
      : null;

    const { data: ins, error } = await sb.from("batch_quality_certificates").insert({
      batch_id: b.id, admin_id: b.admin_id,
      ...
      status, uploaded_by: context.userId,
    } as never).select("id").single();
    if (error) throw error;

    void emitNotification(sb, {
      recipientId: b.admin_id as string, tenantAdminId: b.admin_id as string,
      category: "ops", severity: status === "failed" ? "warning" : "info",
      title: `Quality certificate ${status}`,
      body: `Batch ${b.batch_id ?? b.id}: ${data.fileName}`,
      link: `/silos`, entityType: "grain_batch", entityId: b.id as string,
    });
...
export const verifyCertificate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: role } = await sb.rpc("get_my_role", { _user_id: context.userId });
    if (role !== "super_admin") throw new Error("Forbidden");
    const { error } = await sb.from("batch_quality_certificates").update({
      verified_at: new Date().toISOString(),
      verified_by: context.userId,
      status: data.approved ? "verified" : "rejected",
      verification_note: data.note ?? null,
    } as never).eq("id", data.certificateId);
    if (error) throw error;
    return { ok: true };
  });
...
export const listPendingCertificates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: role } = await sb.rpc("get_my_role", { _user_id: context.userId });
    if (role !== "super_admin") throw new Error("Forbidden");
    const { data: rows } = await sb.from("batch_quality_certificates")
