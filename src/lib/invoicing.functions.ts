...
export const generateInvoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: order, error } = await context.supabase
      .from("buyer_orders")
      .select(
        "id, admin_id, buyer_id, batch_id, order_number, quantity_kg, unit_price, subtotal, currency, buyers(name, company_name, contact_email, contact_phone, address, city, country), grain_listings(title, grain_batches(batch_number))",
      )
      .eq("id", data.orderId)
      .single();
    ...
    const inv = await insertInvoiceWithUniqueNumber<Row>(
      context.supabase,
      o.admin_id as string,
      (invoiceNumber) =>
        context.supabase
          .from("buyer_invoices")
          .insert({
            ...
            total_amount: Number(o.subtotal),
            currency: o.currency,
  ...
  .handler(async ({ data, context }) => {
    ...
    if (fullyPaid && i.order_id) {
      ...
      await context.supabase.from("buyer_order_events").insert({
        ...
        to_state: "paid",
        actor_user_id: context.userId,
        note: `Payment received (${data.paymentMethod})`,
      } as never);
    }
...
export const getSalesSummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const [listingsRes, ordersRes, invoicesRes] = await Promise.all([
      context.supabase.from("grain_listings").select("id, status, available_kg, price_per_kg"),
      context.supabase.from("buyer_orders").select("id, status, subtotal, currency, created_at"),
      context.supabase
        .from("buyer_invoices")
    ...
    const revenue30d = invoices
      .filter(
        (i) =>
          ...
          new Date(i.created_at as string) > new Date(Date.now() - 30 * 86400000),
      )
