...
export const initiateRefund = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rec, error } = await sb.from("buyer_refunds").insert({
      order_id: o.id, admin_id: o.admin_id, dispute_id: data.disputeId ?? null,
      amount, currency: o.currency, reason_key: data.reasonKey,
    ...
    await sb.from("buyer_order_events").insert({
      ...
      from_state: o.status, to_state: amount >= Number(o.subtotal) ? "refunded" : o.status,
      actor_user_id: context.userId,
      note: `Refund ${refund.status} (${o.currency} ${amount}) — ${reason.label}`,
    } as never);
    await logActivity({
      actorId: context.userId, tenantAdminId: o.admin_id as string,
      action: "order.refunded", targetType: "buyer_order", targetId: o.id,
...
export const cancelOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const sb = context.supabase as any;
    const { data: order } = await sb.from("buyer_orders")
      .select("id, admin_id, status, buyer_id").eq("id", data.orderId).single();
    const o = order as Row | null;
    if (!o) throw new Error("Order not found");
    ...
    await sb.from("buyer_order_events").insert({
      order_id: o.id, admin_id: o.admin_id,
      from_state: o.status, to_state: "cancelled",
      actor_user_id: context.userId, note: data.reason ?? "Cancelled",
    } as never);
    await logActivity({
      actorId: context.userId, tenantAdminId: o.admin_id as string,
      action: "order.cancelled", targetType: "buyer_order", targetId: o.id,
