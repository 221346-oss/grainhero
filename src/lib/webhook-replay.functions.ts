...
async function loadOps(sb: any): Promise<OpsCfg> {
  const { data } = await sb.from("platform_settings").select("config").eq("id", "singleton").maybeSingle();
...
export const safeReplayWebhook = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: ev, error: evErr } = await sb.from("insurance_webhook_events").select("*").eq("id", data.event_id).single();
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const res: any = await (replayInsuranceWebhookEvent as any)({ data: { event_id: data.event_id } });
    const success = !!res?.ok;

    const nextEntry = { at: new Date().toISOString(), ok: success, actor: context.userId, error: success ? null : (res?.result?.error ?? "unknown") };
    const newHistory = [nextEntry, ...history].slice(0, 50);
    const failuresInWindow = newHistory.slice(0, cfg.webhook_cooldown_failures).filter((h) => !h.ok).length;
    const nextAllowed = failuresInWindow >= cfg.webhook_cooldown_failures
...
export const getReplayHistory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = context.supabase as any;
    const { data: ev } = await sb.from("insurance_webhook_events").select("replay_count,last_replay_at,next_replay_allowed_at,replay_history").eq("id", data.event_id).maybeSingle();
