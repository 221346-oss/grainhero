...
export const createMobilePaymentIntent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (totalCents < cfg.min_order_cents)
      throw new Error(`amount_below_min:${cfg.min_order_cents}`);
    if (totalCents > cfg.max_order_cents)
      throw new Error(`amount_above_max:${cfg.max_order_cents}`);
    ...
    const pi = (await stripeFetch("/payment_intents", stripeForm(params))) as unknown as {
      id: string;
    ...
    return {
      ...
      publishable_key:
        cfg.stripe_publishable_key_override ?? process.env.STRIPE_PUBLISHABLE_KEY ?? null,
      amount_cents: totalCents,
      currency,
...
export const confirmMobileOrderPaid = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const pi = (await stripeFetch(
      `/payment_intents/${data.payment_intent_id}`,
      null,
      "GET",
    ...
    const { data: orderRaw } = await supabaseAdmin
      .from("buyer_orders")
      .select("id, buyer_id, status, admin_id")
      .eq("id", data.order_id)
      .maybeSingle();
    const order = orderRaw as unknown as {
      id: string;
