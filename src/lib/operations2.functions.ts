...
function req(r: string, allowed: string[]) {
...
export const getMaintenanceOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data: input }) => {
    ...
    const totals = {
      ...
      overdue: list.filter(
        (d) => d.next_maintenance_date && new Date(d.next_maintenance_date).getTime() < now,
      ).length,
      ...
      warrantyExpiring: list.filter(
        (d) => d.warranty_expiry && new Date(d.warranty_expiry).getTime() <= soon,
      ).length,
    };
...
export const getPlatformMaintenanceOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<PlatformMaintenanceOverview> => {
    ...
    const byTenant = new Map<
      string,
      { devices: number; overdue: number; dueSoon: number; lowBattery: number }
    >();
    for (const d of list) {
    ...
    const totals = {
      devices: list.length,
      overdue:
        tenants.reduce((s, t) => s + t.overdue, 0) +
        Array.from(byTenant.values())
          .slice(25)
          .reduce((s, t) => s + t.overdue, 0),
    ...
    return {
      devices: list.map((d) => ({
        ...
        secondsSinceHeartbeat: d.last_heartbeat
          ? Math.round((now - new Date(d.last_heartbeat).getTime()) / 1000)
          : null,
      })),
      totals: {
        total: list.length,
...
export const getSecurityOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [rolesRes, profilesRes, logsRes, managerLogsRes] = await Promise.all([
      ...
      context.supabase
        .from("profiles")
        .select("id, name, email, blocked, last_login, admin_id, created_at")
        .limit(1000),
      ...
      context.supabase
        .from("activity_logs")
        .select(
          "id, action, entity_type, entity_id, user_id, severity, message:description, metadata, created_at",
        )
    ...
    if (managerLogsRes.error)
      throw new Error(`Failed to load manager action logs: ${managerLogsRes.error.message}`);
