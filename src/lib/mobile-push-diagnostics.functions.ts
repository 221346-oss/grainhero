...
export const listPushDeliveries = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await supabaseAdmin
      ...
      .select(
        "id, notification_id, channel, provider, status, error, provider_message_id, attempts, created_at",
      )
      .eq("channel", "push")
...
export const listRegisteredDevices = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await supabaseAdmin
      .from("mobile_devices")
      .select(
        "id, user_id, platform, app_version, last_seen_at, last_push_success_at, last_push_error, last_push_error_at, revoked_at",
      )
...
export const sendTestPush = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { sendPush, markDeviceRevoked, markDeviceSuccess, markDeviceError } =
      await import("./push-dispatch.server");
    ...
    if (res.ok) {
      ...
      return { status: "sent", message_id: res.messageId };
    }
