...
export const getDispatchAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: ships, error } = await sb
      ...
      .select(
        "id, status, courier_key, courier_label, dispatched_at, expected_delivery_at, delivered_at, order_id, buyer_orders(currency, subtotal)",
      )
    ...
    for (const s of rows) {
      ...
      const c = (byCourier[key] ??= {
        key,
        label: String(s.courier_label || key),
        total: 0,
        delivered: 0,
        overdue: 0,
        avgHours: 0,
      ...
      const expected = s.expected_delivery_at
        ? new Date(s.expected_delivery_at as string).getTime()
        : null;
...
export const exportDispatchCsv = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const q = sb
      .from("buyer_shipments")
      .select(
        "id, status, courier_key, courier_label, tracking_number, dispatched_at, expected_delivery_at, delivered_at, notes, order_id, buyer_orders(order_number, currency, subtotal, batch_id, grain_listings(silo_id))",
      )
      .gte("dispatched_at", since)
      .order("dispatched_at", { ascending: false })
      .limit(2000);
    ...
    if (data.siloId)
      filtered = filtered.filter((r) => r.buyer_orders?.grain_listings?.silo_id === data.siloId);
    ...
    for (const r of filtered) {
      ...
      const transitH =
        dispatched && delivered ? ((delivered - dispatched) / 3_600_000).toFixed(2) : "";
      const onTime =
        dispatched && delivered ? (!expected || delivered <= expected ? "yes" : "no") : "";
      lines.push(
        [
          r.id,
          r.buyer_orders?.order_number ?? "",
          r.status,
          r.courier_label ?? r.courier_key ?? "",
          r.tracking_number ?? "",
          r.dispatched_at ?? "",
          ...
          transitH,
          onTime,
          r.buyer_orders?.currency ?? "",
          r.buyer_orders?.subtotal ?? "",
        ]
          .map(escape)
          .join(","),
      );
