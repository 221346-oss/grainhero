...
export const transitionOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: order } = await context.supabase
      .from("buyer_orders")
      .select("id, admin_id, status, batch_id, quantity_kg, listing_id")
      .eq("id", data.orderId)
      .single();
    ...
    const { error: e1 } = await context.supabase
      .from("buyer_orders")
      .update({ status: data.toState } as never)
      .eq("id", data.orderId);
    if (e1) throw e1;
    await context.supabase.from("buyer_order_events").insert({
    ...
    if (o.batch_id) {
      const target =
        data.toState === "dispatched" ? "dispatched" : data.toState === "completed" ? "sold" : null;
      if (target) {
...
export const listOrders = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    let q = context.supabase
      .from("buyer_orders")
      .select(
        "*, buyers(id, name, company_name, contact_email), grain_listings(id, title, currency)",
      )
  ...
  .handler(async ({ data, context }) => {
    const [orderRes, eventsRes, invoiceRes] = await Promise.all([
      context.supabase
        .from("buyer_orders")
        .select(
          "*, buyers(*), grain_listings(*, grain_batches(id, batch_number, grain_type, quality_grade))",
        )
        .eq("id", data.orderId)
        .single(),
      context.supabase
