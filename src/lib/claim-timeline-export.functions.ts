...
export const exportClaimTimeline = createServerFn({ method: "POST" })
  ...
  .validator((d) =>
    z
      .object({ claim_id: z.string().uuid(), format: z.enum(["csv", "pdf"]).default("csv") })
      .parse(d),
  )
  .handler(async ({ data, context }) => {
    ...
    if (data.format === "csv") {
      ...
      return {
        format: "csv" as const,
        filename: `claim_${claim.id}_timeline.csv`,
        content_base64: b64,
      };
    }
    ...
    let y = 800;
    page.drawText("Insurance Claim Timeline", {
      x: 40,
      y,
      size: 18,
      font: bold,
    ...
    for (const e of (events ?? []) as any[]) {
      ...
      page.drawText(`${new Date(e.created_at).toISOString().replace("T", " ").slice(0, 19)}`, {
        x: 40,
        y,
        size: 9,
        font,
    ...
    return {
      format: "pdf" as const,
      filename: `claim_${claim.id}_timeline.pdf`,
      content_base64: b64,
    };
  });
