...
async function ensureBuyerAccount(context: { supabase: unknown; userId: string; claims: Record<string, unknown> }): Promise<Row> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const sb = context.supabase as any;
  const { data: existing } = await sb.from("buyer_accounts").select("*").eq("user_id", context.userId).maybeSingle();
  if (existing) return existing as Row;
  const email = (context.claims as { email?: string })?.email ?? null;
  const { data: created, error } = await sb.from("buyer_accounts")
...
export const updateMyBuyerAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.defaultShippingAddress !== undefined) patch.default_shipping_address = data.defaultShippingAddress;
...
export const createBuyerOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: listing, error: lerr } = await sb
      .from("grain_listings")
      .select("id, admin_id, batch_id, price_per_kg, available_kg, min_order_kg, currency, status, visibility, title")
    ...
    if (data.quantityKg > Number(l.available_kg ?? 0)) throw new Error("Quantity exceeds available stock");
    ...
    if (!buyerId) {
      const { data: newBuyer, error: berr } = await sb.from("buyers").insert({
        admin_id: l.admin_id,
      ...
      await sb.from("buyer_accounts").update({ buyer_id: buyerId } as never).eq("id", account.id);
    }

    // Order number via admin scope count (best-effort; may 0 without perms)
    const { count } = await sb.from("buyer_orders")
      .select("id", { count: "exact", head: true }).eq("admin_id", l.admin_id);
    ...
    const { data: order, error: oerr } = await sb.from("buyer_orders").insert({
      admin_id: l.admin_id,
      buyer_id: buyerId,
      listing_id: l.id,
      batch_id: l.batch_id,
...
export const listMyOrders = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = context.supabase as any;
    const { data, error } = await sb.from("buyer_orders")
      .select("id, order_number, status, quantity_kg, unit_price, subtotal, currency, created_at, checkout_url, paid_at, dispatched_at, completed_at, grain_listings(title, cover_image_url)")
...
export const cancelMyOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = context.supabase as any;
    const { data: o } = await sb.from("buyer_orders").select("id, admin_id, status")
      .eq("id", data.orderId).eq("buyer_account_id", account.id).maybeSingle();
    const row = o as Row | null;
    if (!row) throw new Error("Order not found");
    if (row.status !== "pending") throw new Error("Only pending orders can be cancelled");
    const { error } = await sb.from("buyer_orders").update({ status: "cancelled" } as never).eq("id", data.orderId);
    if (error) throw error;
    await sb.from("buyer_order_events").insert({
      order_id: data.orderId, admin_id: row.admin_id,
      from_state: "pending", to_state: "cancelled",
      actor_user_id: context.userId, note: "Cancelled by buyer",
    } as never);
    return { ok: true };
  });