...
export const requestReturn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (order.buyerUserId !== context.userId)
      throw new Error("Only the buyer can request a return");
    ...
    if (daysSince > settings.returns.windowDays)
      throw new Error(`Return window (${settings.returns.windowDays}d) has closed`);
    ...
    void emitNotification(sb, {
      recipientId: order.admin_id as string,
      tenantAdminId: order.admin_id as string,
      category: "order",
      severity: "warning",
      title: `Return requested on order ${order.order_number ?? ""}`.trim(),
      body: `${reason.label}${data.notes ? ` — ${data.notes}` : ""}`,
      link: `/returns`,
      entityType: "buyer_return",
      entityId: rid,
    });
    try {
...
export const approveReturn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await insertEvent(sb, {
      ...
      fromState: ret.status,
      toState: "approved",
      actorUserId: context.userId,
      actorRole: role,
      note: data.note ?? resolution.label,
    });

    if (ret.buyer_user_id) {
      void emitNotification(sb, {
        recipientId: ret.buyer_user_id as string,
        tenantAdminId: ret.admin_id as string,
        category: "order",
        severity: "success",
        title: `Return approved`,
        body: resolution.label,
    ...
    try {
      ...
      await sendBuyerOrderEmail(
        context.supabase,
        ret.order_id as string,
        "returnApproved" as never,
      );
    } catch {
      /* noop */
    }
  ...
  .handler(async ({ data, context }) => {
    ...
    if (ret.buyer_user_id) {
      void emitNotification(sb, {
        ...
        severity: "warning",
        title: `Return denied`,
        body: data.note ?? "See order for details.",
        link: `/buyer/orders/${ret.order_id}`,
        entityType: "buyer_return",
        entityId: data.returnId,
      });
  ...
  .handler(async ({ data, context }) => {
    ...
    if (ret.buyer_user_id) {
      void emitNotification(sb, {
        ...
        severity: "success",
        title: `Refund issued`,
        body: `Refund processed for your return.`,
        link: `/buyer/orders/${ret.order_id}`,
        entityType: "buyer_return",
        entityId: data.returnId,
      });
