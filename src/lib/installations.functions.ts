  ...
  .handler(async ({ data, context }) => {
    ...
    const [
      ...
      supa
        ...
        .select(
          "id, admin_id, install_address, install_city, install_country, contact_phone, technician_name, technician_phone, status, plan_name, hardware_quantity, hardware_total, created_at, scheduled_install_date",
        )
        .eq("id", data.orderId)
        .maybeSingle(),
      supa
    ...
    const company =
      (settings?.config as { company?: { origin_address?: string } } | null)?.company ?? {};
    return {
      order,
      ...
      events: events ?? [],
      companyOrigin:
        company.origin_address ?? "J453+GPQ, Old Airport Rd, Chaklala Cantt., Rawalpindi, 46000",
    };
  });
...
export const upsertInstallation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!patch.origin_address || !patch.origin_address.trim()) {
      ...
      patch.origin_address =
        co.origin_address ?? "J453+GPQ, Old Airport Rd, Chaklala Cantt., Rawalpindi, 46000";
    }
...
export const upsertDevices = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: { orderId: string; devices: { serial: string; model?: string; status?: string }[] }) => d,
  )
  .handler(async ({ data, context }) => {
    if (!(await isSuperAdmin(context))) throw new Error("Forbidden: super_admin only");
    const supabaseAdmin = context.supabase;
...
export const addVisitEvent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    if (!(await isSuperAdmin(context))) throw new Error("Forbidden: super_admin only");
    const supabaseAdmin = context.supabase;
    const { error } = await supabaseAdmin.from("hardware_order_visit_events").insert({
      order_id: data.orderId,
      note: data.note,
...
export type InstallStage =
  "paid" | "assigned" | "en_route" | "onsite" | "installed" | "completed" | "blocked";
