...
export const createShipment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: order } = await sb.from("buyer_orders")
      .select("id, admin_id, status, shipment_id").eq("id", data.orderId).single();
    ...
    const trackingUrl = resolveTrackingUrl(courier.trackingUrlTemplate, data.trackingNumber ?? null);

    const nowIso = new Date().toISOString();
    const { data: ship, error } = await sb.from("buyer_shipments").insert({
      order_id: data.orderId, admin_id: o.admin_id,
      courier_key: courier.key, courier_label: courier.label,
      tracking_number: data.trackingNumber ?? null, tracking_url: trackingUrl,
      status: "in_transit", dispatched_at: nowIso,
    ...
    await sb.from("buyer_orders").update({
      shipment_id: shipmentId, status: "dispatched",
    } as never).eq("id", data.orderId);
    await sb.from("buyer_order_events").insert({
      order_id: data.orderId, admin_id: o.admin_id,
      from_state: "paid", to_state: "dispatched",
    ...
    await logActivity({
      ...
      action: "shipment.created", targetType: "buyer_shipment", targetId: shipmentId,
...
export const markDelivered = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const sb = context.supabase as any;
    const { data: ship } = await sb.from("buyer_shipments")
      .select("id, order_id, admin_id, status").eq("id", data.shipmentId).single();
    const s = ship as Row | null;
    if (!s) throw new Error("Shipment not found");
    const nowIso = new Date().toISOString();
    await sb.from("buyer_shipments").update({
      status: "delivered", delivered_at: nowIso,
    } as never).eq("id", data.shipmentId);
    await sb.from("buyer_shipment_events").insert({
    ...
    const { data: order } = await sb.from("buyer_orders")
      .select("id, admin_id, status, batch_id").eq("id", s.order_id).single();
    const o = order as Row | null;
    if (o && o.status !== "completed") {
      await sb.from("buyer_orders").update({
        status: "completed", delivered_at: nowIso,
      } as never).eq("id", o.id);
      await sb.from("buyer_order_events").insert({
        order_id: o.id, admin_id: o.admin_id,
        from_state: o.status, to_state: "completed",
        actor_user_id: context.userId, note: data.note ?? "Marked delivered",
      } as never);
      if (o.batch_id) {
        await sb.from("grain_batches").update({
          status: "sold", state_changed_at: nowIso,
    ...
    await logActivity({
      ...
      action: "shipment.delivered", targetType: "buyer_shipment", targetId: data.shipmentId,
...
export const getShipmentTracking = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = context.supabase as any;
    const { data: shipment } = await sb.from("buyer_shipments")
      .select("*").eq("order_id", data.orderId).maybeSingle();
    if (!shipment) return { shipment: null, events: [] };
    const { data: events } = await sb.from("buyer_shipment_events")
      .select("*").eq("shipment_id", (shipment as Row).id).order("at", { ascending: true });
