  ...
  .handler(async ({ data, context }) => {
    ...
    if (recipientId) {
      void emitNotification(sb, {
        recipientId,
        tenantAdminId: parties.adminId,
        category: "order",
        severity: modReason ? "warning" : "info",
        title: `New message on order ${parties.orderNumber ?? ""}`.trim(),
        body: data.body.slice(0, 240),
        link:
          senderRole === "buyer"
            ? `/platform/orders/${data.orderId}`
            : `/buyer/orders/${data.orderId}`,
...
export const markMessagesRead = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await sb
      ...
      .is(col, null);
    const zero =
      data.as === "seller" ? { unread_seller_messages: 0 } : { unread_buyer_messages: 0 };
    await sb
      .from("buyer_orders")
      .update(zero as never)
      .eq("id", data.orderId);
    return { ok: true };
  });
    ...
    const patch =
      data.action === "approve"
        ...
        : {
            ...
            moderation_reason: data.reason ?? "hidden by super_admin",
          };
