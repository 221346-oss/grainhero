...
export interface LedgerEntryInput {
  entryType: "payment_in" | "refund_out" | "platform_fee" | "logistics_cost" | "tax" | "payout_out" | "adjustment";
...
export async function writeLedgerEntry(
  ...
  const { data, error } = await sbAny.from("finance_ledger_entries").insert({
...
function centsFromMinor(v: number) { return Math.round(v * 100) / 100; }
...
export const getSellerBalance = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows } = await sb.from("finance_ledger_entries")
      .select("amount, direction, status, currency, entry_type")
      .eq("seller_id", sellerId);
    const list = (rows ?? []) as Row[];
    let payable = 0, onHold = 0, paidOut = 0, lifetimeGross = 0, refunded = 0, fees = 0;
    for (const r of list) {
      ...
      if (r.entry_type === "platform_fee" || r.entry_type === "logistics_cost") fees += Number(r.amount ?? 0);
...
export const getPlatformFinanceSummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const list = (rows ?? []) as Row[];
    let gmv = 0, refunds = 0, platformFees = 0, tax = 0, logistics = 0, payoutsPaid = 0;
    ...
    for (const r of list) {
      ...
      if (r.entry_type === "payment_in") { gmv += amt; daily[day].gmv += amt; }
      else if (r.entry_type === "refund_out") { refunds += amt; daily[day].refunds += amt; }
      else if (r.entry_type === "platform_fee") { platformFees += amt; daily[day].fees += amt; }
    ...
    // pending payouts
    const { data: pending } = await sb.from("seller_payouts")
      .select("net_amount").in("status", ["pending","approved","processing"]);
    const pendingPayouts = ((pending as Row[]) ?? []).reduce((s, r) => s + Number(r.net_amount ?? 0), 0);
...
export const listLedgerEntries = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: isAdmin } = await context.supabase
      .rpc("is_super_admin", { _user_id: context.userId });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q: any = (context.supabase as any).from("finance_ledger_entries")
      .select("id, entry_type, direction, amount, currency, seller_id, order_id, status, occurred_at, metadata")
